...
能够方便地把数据以图形化的方式呈现出来，是 LabVIEW 的一大特色。LabVIEW 提供了丰富的图形控件。对于最常用的二维图形控件（如波形图、波形图表、XY 图），它们的使用非常直接：拖放到前面板后，在程序框图中直接连接相应的数据即可显示，无需额外的辅助代码。

不过，对于较复杂的 三维图形控件 (3D Graphs)，为了简化配置过程，LabVIEW 在将它们拖放到前面板时，确实会在程序框图上自动添加一些辅助 VI（Helper VIs）。这些辅助代码用于帮助用户构建三维场景和数据结构。
...
* 波形图表控件正相反，它接受的二维数组数据中，每一列代表一条曲线（Column per Channel）。

由于很多数据采集驱动（如 NI-DAQmx）读取出来的二维数组默认是“每一行代表一个通道”，因此在将多通道采集数据直接送给波形图表显示之前，通常需要加一个 “二维数组转置 (Transpose 2D Array)” 函数，否则显示的波形将毫无意义。
...
到目前为止，我们一直在使用数组的索引来表示曲线的时间信息，但在很多实际应用中，是需要显示曲线的实际时间信息的。比如，如果一段数据采集于 2021 年 12 月 18 日上午 9 点 01 分 01 秒至 2021 年 12 月 18 日上午 9 点 01 分 02 秒，这些时间信息需要被显示在波形图的横轴上以方便用户理解数据。这就需要把时间信息和数据的值一同传递给波形控件才行。在 LabVIEW 中，我们可以把时间信息和波形数据一起打包成一个簇（Cluster）数据类型。不过，我们不需要自己定义这个簇了，LabVIEW 已经定义了一个特殊的簇类型，专用于保存带时间的波形数据，这就是波形数据类型。
...
XY 图形控件不接受两个独立的数据连线。在程序框图中，我们需要使用 “捆绑 (Bundle)” 函数，将表示 X 轴的一维数组和表示 Y 轴的一维数组打包成一个簇 (Cluster)，或者将包含 X 和 Y 坐标的点簇组成数组，然后再连接到 XY 图形控件上。
...
由于 LabVIEW 自带的函数中没有计算二维数据分布图的 VI，这个示例程序稍微麻烦了一点，需要自己计算数据在每个身高体重区间内的分布。我们把身高和体重都各自等分了 200 份，也就是把数据按照身高体重组成了一个 200*200 的网格，然后把每个人的数据放到相应的网格里。在 XY 图形控件中用亮度来表示每个网格中人数的多少：

![images_2/z005.png](images_2/z005.png "身高和体重的分布图")

上图看上去有点像斜着放的银河系，但是可以看出来，有两个亮度比较明亮的区域，分别是男性和女性中最常见的身高体重区域。
...
图片使用的坐标系与 LabVIEW 中二维图形控件使用的坐标系略有不同，LabVIEW 图形控件多是使用的标准的直角坐标系，是把左下角当作原点，向右为 X 轴正向，向上为 Y 轴正向；图片中一般把左上角作为原点进行计数，向右为 X 轴正向，向下为 Y 轴正向。

这个示例程序没有调整读入数据的方向，所以在 LabVIEW 强度图中显示的图片是翻转了 90 度的。与原图对比一下：

![images_2/labview.png](images_2/labview.png "一个 LabVIEW 图标")

图片文件读取后得到的通常是一个“行-列”结构的二维数组。LabVIEW 的强度图默认将数组的第一维（行索引）作为 X 轴，第二维（列索引）作为 Y 轴，这与图像数据的逻辑正好相反（图像通常第一维是 Y，第二维是 X），导致图像看起来旋转了 90 度。计算机图像坐标系的原点通常在左上角，而强度图作为数学绘图控件，原点默认在左下角。

若要用强度图正确显示图片，通常需要先对数组进行转置 (Transpose)，甚至可能需要调整坐标轴方向或对数组进行倒序处理，才能得到直立的图像。
