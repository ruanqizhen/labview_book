...
比较两个实数是否相等的时候，要考虑误差。由于计算机中能够表示出来的数值有限，很多数据只能用近似值来表示，这就产生了误差。一般，如果两个实数相差小于某一预定的值（允许多大的误差要根据项目具体情况和要求来设定），就可以算它们是相等的了。同一段程序，在不同的电脑上运算，误差可能也不同，比如下面的程序，在笔者电脑上直接比较 $\sqrt{-1}$ 的实数部分是否是 0，结果却是假。

![](images_2/z142.png "检查实数是否相等")

无穷大与无穷大是相等的，无穷大加减乘除某个具体的数也还是无穷大（或者负无穷大）。一个数据，加上 100 之后的结果在和原数据比较是否相等，结果是不是应该永远为“假”？在有些编程语言中是的，但在 LabVIEW 中却不一定，比如：
