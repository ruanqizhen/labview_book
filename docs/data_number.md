...
无穷大与无穷大是相等的，无穷大加减乘除某个具体的数也还是无穷大（或者负无穷大）。一个数值 x 是不是肯定不等于 x+100？在数学定义中是的，在很多编程语言中也是的，但在 LabVIEW 中却不一定，比如：

![](images_2/z143.png "无穷大还是无穷大")

NaN 与任何数都不想等，包括它自己。如果要检查一个数据是否是 NaN 只能使用“不是一个数字/路径/引用”函数来检查：
