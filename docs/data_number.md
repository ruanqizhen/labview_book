...
打开这个控件的属性对话框，在显示格式页面中，选择"高级编辑模式"，就可以为控件设置显示方式了（图
2.2）。例如，在"格式字符串"一栏中输入"%\<%Y-%m-%d
%H:%M:%S>T"，可以让控件按照"年-月-日
时:分:秒"的格式把一个实数数值显示出来（图
2.3）。而这个实数用来表示时间时，其含义是指1904-01-01
08:00:00这一时刻之后的多少秒。
...
![](images/image67.png){width="1.6875in" height="1.8125in"}
