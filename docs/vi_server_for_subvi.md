...
虽然动态装载可以优化内存和启动时间，但在现代计算机硬件下，对于绝大多数中小型项目，静态装载依然是绝对的首选。滥用动态装载（“打开 VI 引用”）会带来严重的副作用：
1. 破坏代码可读性： 无法直观地看到数据流和模块依赖。
2. 重构噩梦： LabVIEW 的“查找调用者 (Find Callers)”等重构工具无法追踪动态调用的 VI。
3. 打包风险： 在生成 EXE 时，LabVIEW 无法自动识别被动态引用的子 VI，开发者极易忘记将其手动添加到依赖项中，导致 EXE 运行时报错“找不到 VI”。

只有针对极少使用且包含极其庞大资源的模块（如报表生成模块、庞大的硬件驱动库）或插件架构（Plugin Architecture），才建议使用动态装载。
...
我们也可以不用在启动画面程序中编写循环去逐个打开文件夹里的子 VI，这既繁琐又容易遗漏依赖项。我们可以利用 LabVIEW 的原生依赖加载机制：

将“启动画面 VI (Splash.vi)”设置为整个项目（或 EXE）的唯一启动 VI。

在 Splash.vi 运行显示进度界面的同时，使用“打开 VI 引用”函数（开启异步调用），动态指定并打开真正的“主程序 VI (Main.vi)”。

核心机制： 当“打开 VI 引用”节点试图加载 Main.vi 时，LabVIEW 引擎会自动将 Main.vi 依赖的所有静态子 VI 一并加载到内存中。这个过程正是程序启动最耗时的部分。

等 Main.vi 被异步启动且前面板打开后，Splash.vi 就可以关闭自身的前面板并退出了。
...
在 LabVIEW 2011 之前，实现主程序功能的唯一方法是使用 VI 的“运行”方法节点，把“维持表盘转动 VI”动态调用运行起来，其程序框图如下图所示。

![image](images/image427.png "采用动态调用方法的程序框图")

这段代码最不尽人意的地方在于，运行子 VI 之前先要通过 VI 的 "设置控件值" 方法为子 VI 传递参数。这种方法不但编写复杂，安全性也低。首先，它是通过控件名来找到需要传递参数的控件的，LabVIEW 无法在编译时检查参数名字是否正确。子 VI 中的控件若是修改了名字，程序编译时不会发现问题，但运行时就会出错。其次，它无法检查需要传递的参数的数据类型，即使传递的是一个错误类型的数据，在编译时也发现不了错误。
...
异步调用分为两种模式：
1. 调用并忘记 (Call and Forget)： 如果启动子 VI 后不需要等待它结束，也不需要获取它的输出结果（如本文的仪表盘例子）。必须传入十六进制数值 0xC0（即 0x80 准备异步调用 + 0x40 调用并忘记）。这样当子 VI 运行结束后，系统会自动清理其内存实例，防止内存泄漏。
2. 调用并收集 (Call and Collect)： 如果打算在未来的某个时刻使用“等待异步调用”节点来获取它的输出结果，则只需传入 0x80。
...
在“开始异步调用”函数的同一函数选板上，还有一个“等待异步调用”，它用于并行任务分发与结果收集（例如：分配 10 个线程去分析 10 个不同的数据文件，最后汇总结果）。与“开始异步调用”函数相比，“等待异步调用”函数的实际用途有限，并不常见。因为，如果需要等待子 VI 运行结果的话，异步和同步的区别就不那么大了。
