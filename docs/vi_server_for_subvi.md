...
有两种方法可以完成这个功能。一是使用“通过引用节点调用”节点。上一段在讲述通过子 VI 的“调用设置”选项来设定 VI 的装载方式时，提到过这个节点。我们还可以采用另外一种方法，即先把这个节点放到程序框图上，再把一个动态打开的 VI 引用传递给这个节点的“引用”输入，就可以动态打开并运行这个 VI。拖放到程序框图上的这个节点与通过“调用设置”选项获得的节点的图标也是有细微区别的，前者的上方对应输入输出引用端口处，各有一个小小的象征“引用”的图案：
