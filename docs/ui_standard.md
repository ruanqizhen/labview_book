...
使用分隔栏可以在前面板上分割出更多的窗格来。分隔栏在控件选板各风格的“容器”子选板中（如“新式 -> 容器”）。分隔栏有横、竖之分，可以把前面板按照不同的方向分成两块。控件无论被放在哪个窗格里，只是界面的外观不同，对程序功能都没有影响。
...
![](images/image595.png "使用分隔栏的位置属性调整分隔栏位置")
...
![](images/image600.png "将多个控件设置为一组")
...
![](images/image603.png "每个分隔栏的对齐方式")
...
2. **有些控件必须保持长宽比** 大多数 LabVIEW 控件可以分别设置它们的高度和宽度。但有一些控件，比如下图中的仪表盘、旋钮等，它们的形状是固定的，只能是正圆。也就是无论大小如何，长宽始终保持 1:1。
...
![](images/image607.png "关闭程序窗口，需要相应地终止程序运行")
...
这里的核心关键在于事件结构节点左侧的 “放弃? (Discard?)” 布尔接线端。如果用户点击“取消”（不想退出），不仅不能执行停止循环的代码，必须将“放弃?”端子连线为 True (T)。这会告诉 LabVIEW 引擎：“拦截这次关闭操作，保持窗口打开”。

如果用户点击“确定”，则执行资源清理和停止循环的代码，可以将“放弃?”端子连线为 False (F) 允许窗口关闭，或者保持 True 自己通过逻辑结束程序。如果不连线“放弃?”端子（默认是 False），那么只要用户点了红叉，无论对话框选什么，窗口都会瞬间消失。
...
表格是一种常见的界面控件，本例中，用户需要表格中某一列的内容只在有限的范围内选择。实现这样的表格，可以用一个下拉菜单列表控件给用户提供选择内容，以限定用户输入内容的范围，防止填错。比如下图所示的一个程序界面，界面上表格的第二列需要由用户填写，并且只能在“甲”“乙”“丙”“丁”四个值中选择。当用户点击表格第二列上某个单元格时，该单元格会变成一个下拉菜单列表控件，让用户从中选取一个值。
...
上述通过事件动态移动下拉框覆盖单元格的方法，是早期 LabVIEW 开发者常用的“黑客（Hack）”技巧。但这种方法极其脆弱：一旦用户滚动了列表框的滚动条，或者改变了窗口大小，覆盖的下拉框坐标计算就会非常复杂甚至错位。

如果需要实现单元格级别的复杂交互（如下拉选择、复选框打勾），可以放弃多列列表框，改用树形控件。尽管名字叫“树”，但如果不建立层级关系，它看起来就是一个带网格的表格。更强大的是，树形控件原生支持通过属性节点为特定的单元格或整列直接配置“自定义符号 (Custom Symbols)”或让其表现得像下拉列表。这比自己写代码追踪坐标要稳健、专业得多。
