...
在前文中，我们已经详细介绍了面向对象编程的三大特性：[封装](oop_basic#封装数据抽象)、[继承](oop_basic#继承)、[多态](oop_basic#多态动态分派)。在设计程序时，这三大特性依然是最重要的考量因素，也就是说，在设计类时，我们需要考虑类中有哪些属性和方法，是否可以继承其他一些类中的属性和方法等。这三大特性在前面的章节中已经详细介绍过了，我们在下文将着重讲解其他一些比较常用的设计方法和技巧，比如一个类是否需要依赖另一个类，一个类是否由其他一些对象组成的等等。
...
再来来看一个违反里氏替换原则的经典设备控制例子。假设我们有一个父类 Instrument（仪器），它有一个公开方法 Initialize.vi 和一个 Start_Measurement.vi。
我们写了一个子类 Laser_Controller（激光控制器）继承自 Instrument。但是，处于安全考虑，激光控制器在开始测量前，必须人为按下硬件面板上的一个物理安全按钮。如果没按下，软件调用 Start_Measurement.vi 时就会直接弹出一个严重错误弹窗并中断程序。

这种设计就严重违反了里氏替换原则。因为在主程序的测试序列中，主程序期望的是一个普通的 Instrument，它认为只要调用了 Initialize 就可以调用 Start_Measurement。当我们把 Laser_Controller 塞进去替换父类时，主程序的预期被打破了，程序直接崩溃。

为了遵循里氏替换原则，子类在重写（Override）父类方法时，不能加强前置条件（不能要求比父类更苛刻的运行环境），也不能削弱后置条件（不能改变父类承诺的输出结构或抛出未预期的错误）。
...
这个设计的问题在于： ReportService 直接依赖于 PDFStudentReport。当需求发生变动的时候，比如，新的要求是生成一个 Web 格式的报告，我们就必须要修改 ReportService 类。如果我们遵循依赖倒置原则，就可以不修改 ReportService 而直接支持新的报告格式了。实现方法如下：
我们定义一个抽象的 IReport 接口，然后让 PDFStudentReport 和 WebStudentReport 都去实现这个接口。

![](images_2/z349.png "create_report.vi")

关键的改变在于 ReportService 类的私有数据簇（.ctl 文件）：它的内部不再保存 PDFStudentReport 的具体对象，而是保存一个 IReport 接口类型的线缆。

在主程序初始化时，我们通过一个 Set_Report_Engine.vi（这在软件工程中被称为依赖注入 Dependency Injection），将具体的 PDF 或 Web 报告对象的实例传递给 ReportService。以后如果需要增加 XML 格式的报告，ReportService 的核心代码连一根线都不需要修改！


## 单一功能原则

单一功能原则，是指，一个类应该有且仅有一个引起它变化的原因（Reason to change）。简单的说就是一个类只做一件事。
...
再举一个例子：假设我们在编写一个自动化测试程序，我们设计了一个 Test_Manager 类。这个类里面既写了控制万用表读取电压的代码，又写了将电压数据保存到 SQL 数据库的代码，还写了在界面上画曲线图的代码。

这个类就违反了单一功能原则。为什么？因为有三个不同的原因会导致我们需要去修改这个类：
1. 万用表换型号了（硬件通信变化）。
2. 数据库表结构变了（存储逻辑变化）。
3. 客户要求把曲线图换成柱状图（UI 逻辑变化）。

遵循单一功能原则的设计，应该把这三个可能独立变化的轴拆分开来。比如拆分成 DMM_Driver 类（专管硬件）、Database_Logger 类（专管存储）和 UI_Controller 类（专管显示）。这样，无论哪一方的需求发生变更，都不会波及和污染其他核心代码。
...
实现组合 (Composition)： 最简单直接。比如 Car 类由 Engine 类组成。你只需要打开 Car.ctl（汽车类的私有数据簇），直接把 Engine 类的对象常量拖进去即可。当 Car 实例在数据线上流动或被销毁时，内部的 Engine 实例也会完全同步地经历同样的生命周期。
...
比如 Classroom 类包含多个 Student 类。因为学生下课后还要去操场，教室不能负责销毁学生。此时，你绝对不能在 Classroom.ctl 中直接放一个 Student 类的数组。在 LabVIEW 中实现聚合，你必须在 Classroom.ctl 中放入指向学生的 数据值引用 (DVR) 数组，或者保存学生的 唯一 ID 字符串（由外部对象管理器统一管理对象的生命周期）。
