...
而多数文本编程语言难以把其所有函数按照功能分类列好，以供用户随时选用。例如，在用 VC++ 编写程序时，函数需要通过键盘逐字输入。一般情况下，编辑环境只能根据已经敲入的字符做提示。比如敲入两个字母“sq”，系统会提示是不是想要“sqrt”。 假如你的程序中需要把一个浮点数转换为用十六进制表示的字符串，这种常用的功能，肯定可以有现成的函数可用，可就是不知道这个函数名是什么，都无法预先输入前几个字母。于是，不得不翻开各种参考资料查找线索，实在找不到只好自己再重新编写调试一个具有此功能的函数。

若是使用 LabVIEW，这个问题就相对容易解决。初学者也可以大致猜到：如果 LabVIEW 提供了这样一个函数，那么它或是在“数值”函数选板下，或是在“字符串”函数选板下，或是在某个专门的数据转换函数选板下。只要到这几个地方找一找，就能够找到。
...
即时帮助窗口左下方有三个小按钮，读者可以自行探索其功能：点击它们，然后看看即时帮助窗口有什么变化。（提示：尝试的时候，在程序框图上可以多放几个其它的结构、子 VI 等，鼠标在几个不同的节点上移动，观察即时帮助窗口的变化。）
