...
配置好源代码控制后，项目浏览器中的文件图标上会出现状态角标。不同的源代码管理工具（Perforce, SVN, Git 等）显示的图标不同，例如在 Perforce 中，红色的勾通常表示文件已被“签出”并处于可编辑状态。

![](images/image536.png "源代码控制功能")

开发人员可以通过使用项目浏览器的工具条上相关的按钮或项目浏览器中文件的右键菜单对项目中的文件进行源代码管理。比如，可以完成从服务器上签出、把新版本上传至服务器等操作。

虽然 LabVIEW 提供了内置的源代码管理集成，但在处理 Git 或 SVN 等现代分布式版本控制系统时，许多资深开发者更倾向于禁用 LabVIEW 内置集成，转而使用外部的专用客户端（如 TortoiseGit, Sourcetree 或命令行）。这样可以避免 LabVIEW IDE 的卡顿，并能使用更高级的版本控制功能（如分支管理、暂存区等）。
...
虽然 LabVIEW 提供了“合并 VI”工具，但图形化代码的自动合并远比文本代码困难且风险更高。复杂的程序框图冲突往往难以通过工具完美解决。

尽量避免多人同时修改同一个 VI。通过良好的模块化设计（将大 VI 拆分为多个子 VI）来降低冲突概率。如果确实发生了冲突，通常更安全的做法是放弃一方的修改，或者人工手动将变更重新应用到最新版本上，而不是过分依赖自动合并工具。
...
项目除了方便用户进行文件管理之外，还有一个核心功能就是为项目中的 VI 提供了一个独立的 应用程序实例 (Application Instance)。

不同的应用程序实例拥有独立的内存空间。这意味着，即便两个 VI 的文件名和路径完全相同，只要它们分别属于不同的项目（即不同的应用程序实例），它们就可以同时被加载到内存中互不干扰。这对于开发和调试同名 VI 非常有用。
...
使用库 (.lvlib) 的另一个巨大优势是防止交叉链接。在没有库的情况下，LabVIEW 仅凭文件名加载 VI，很容易加载到错误的同名 VI（例如两个项目都用了 utils.vi）。而放入库后，VI 的全名变成了 mylib.lvlib:utils.vi，这使得 LabVIEW 能够精确地链接到正确的文件，极大地增强了代码的稳健性。
...
LLB 是一个单一文件，使用它不利于源代码控制。

LLB 是一个二进制的大包。哪怕你只修改了 LLB 里面 100 个 VI 中的一个微小改动，版本控制系统（如 Git）也会认为整个 LLB 文件都发生了变化。

这会导致版本库体积迅速膨胀（因为每次提交都存一份大的二进制副本）。

你无法查看 LLB 内部的具体变更历史。

因此，在源码开发阶段，坚决杜绝使用 LLB。请直接使用文件夹（磁盘目录）配合库文件（.lvlib）来组织代码。
