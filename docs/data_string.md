...
目前 LabVIEW 是不支持 unicode 的，LabVIEW 在 Windows 上默认使用操作系统的本地编码，例如在中文 Windows 上通常是 GBK 编码。而在 Linux 和 macOS 上，LabVIEW 通常使用 UTF-8 编码。关于这个问题的详细信息可以参考 [LabVIEW 的 unicode 问题](appendix_problem) 一节。

这导致了跨平台时的乱码问题：一个包含中文字符的 VI，在 Windows 上保存后拿到 Linux 上打开，由于编码解释方式不同（GBK vs UTF-8），中文字符会显示为乱码。
:::
...
这是因为 `%s` 默认会读取直到遇到空白字符（空格、Tab、换行）为止的内容，或者读完剩余所有字符。如果两个字符串紧挨在一起且没有分隔符，第一个 `%s` 会“贪婪”地吞掉所有内容，导致第二个 `%s` 无处可配。

解决这个问题通常需要明确的分隔符（如逗号），或者在格式字符串中指定长度（例如 `%4s`），或者改用正则表达式进行更精准的提取。
...
JSON 的效率通常高于 XML，且更易于阅读。LabVIEW 提供了原生的 JSON 平化和还原函数。

注意：LabVIEW 原生的 JSON 函数对数据类型的匹配非常严格（例如，JSON 中的数字必须严格对应 LabVIEW 中的数值控件，不能对应字符串控件），且在处理“变体 (Variant)”等 LabVIEW 特有数据类型时不如 XML 灵活。对于复杂的 JSON 处理需求，社区中也有成熟的第三方工具包（如 JKI JSON）可供选择。
