...
那么两种解决方案哪个好呢？作为初学者来说，使用默认分支会感觉好一些，因为方便，省去了添加代码的麻烦。尤其需要提到的是，当条件是一个 [枚举类型](data_custome_control)  的时候，它的数据值的个数是有限的，我们可以在条件结构中为每个条件都设置好相应的分支。但是，将来程序可能会出现变动，可能会在枚举类型中添加一个新的条目，或改动某个条目的名字。如果条件结构没有默认分支，作为条件的枚举类型一旦改动，VI 就会出错；而如果条件结构有默认分支，则 VI 不会报错，还能运行。

笔者自己的体会是这样的：在初学一个编程语言时，最大的挑战是让程序运行起来，程序能运行就好，怎么方便就怎么来。但是，在开始用一个语言做实际项目的时候，就会体会到，一个有隐患的程序能运行还不如不能运行。因为，如果程序运行返回错误，可以提示程序员去查找修正错误，否则，把程序交给用户后，用户发现它的结果有错，那问题就严重了。最理想状况是，有潜在问题的时候，程序甚至无法编译（也就是 VI 无法运行），这样程序员可以通过错误信息直接找到问题所在。

还是以枚举类型条件为例，程序最开始设计条件结构时，很可能并不知道将来会添加一个什么样的条件值。在新添加一个枚举类型的条目后，很可能需要条件结构对这个新的值做一些特殊处理的。如果 VI 在改动枚举类型后，并不报错，程序员可能也想不到去更新条件结构，这就是个隐患。相反，如果条件结构中没有默认分支，枚举类型被改动后，VI 会报错，就提示了程序员去做修改。

这是一个比较违反初学者直觉的原则：如果程序可能会出错，那么我们会希望它越早出错越好，错误越严重越好，最好能够直接让代码无法编译。
...
由于记录时间的代码部分，与被测试的代码部分并无数据线相连，而它们之间又必须以一定的先后顺序执行，所以可以考虑使用顺序结构。首先使用层叠式顺序结构，它有三个帧：第 0 帧记录下系统当前的时间，也就是测试代码运行前的时间；第 1 帧中的代码是被测试代码；第 2 帧再次读取系统当前时间，这个时间减去第 0 帧读到的时间，就是第二针测试代码运行的时间。
...
在上面的示例程序中，用到的“设置仪器.vi”和“读仪器数据.vi”是两个演示用的子 VI，在标签中已表明其功能；“仪器名称”常量表示的是某一个仪器（读者目前只需知道他代表某一仪器，仪器的具体使用方法本书会在[测试测量应用程序设计](measurement)一章去讲解）；两个子 VI 之间下方的连线是错误数据线；“等待（ms）”是 LabVIEW 自带的用于延时的函数，位于“编程 -\> 定时”函数选板。
