...
泛型编程是一种抽象度更高的编程风格。使用泛型抽象和编写出来的算法，不但不依赖与具体的数据，甚至也不依赖于具体的数据类型。比如，同一个排序函数不但可以给一组整数排序，也可以给一组字符串排序，还可以按大小给一组苹果排序，也可以按成绩给一组学生排序等等。这种实现了与数据类型无关的算法的编程方式就是泛型编程。从这段描述可以看出来：编程语言要支持泛型编程，一个前提条件是，使用这个语言能够编写出一个函数，它的某个参数可以接受不同类型的数据。

LabVIEW 中的很多函数是支持泛型的，比如加法函数，可以接收数值类型的数据，也可以接受簇或者数组类型的数据。再比如数组相关的函数，可以用来操作数值数组，一可以操作字符串数组。

![images_2/z045.png](images_2/z045.png "加法函数可以支持多种数据类型")

如果一个 VI，使用变体作为参数数据类型，这个 VI 的参数就可以接收任何类型的数据了，这样是不是就算作支持泛型编程了？（或者是像 Python 2.0 这样的编程语言，变量不需要有类型定义，任何类型的数据都可以传递给同一个变量，是不是就可以支持泛型编程了？）

这样还不够，要支持泛型编程，编程语言不仅要具备同一参数支持不同数据类型的功能，还要具有静态的类型检查功能才可以。比如，某一算法可以支持数值和字符串类型的数据，但是不支持布尔类型数据，那么就需要实现这样的功能，当把布尔数据类型于这个算法子 VI 的输入连线的时候，VI 就会立刻报错，不需要运行，VI 就可以提示数据类型出错。 子 VI 使用变体数据类型显然无法做到这一点。甚至更复杂的情况，如果创建了一个字符串类型的数组，再使用其它数组函数（比如插入、索引等）对这个数组操作时，函数应当也只能接收字符串类型的数据，连接到其它数据类型也需要报错。
...
在 LabVIEW 中，也有几种方法可以编写出支持多种不同类型数据的 VI，下面逐一介绍。

## 使用变体作为子 VI 的参数类型

我们在[数据类型间的转换](data_datatype_cast)一节曾经介绍过“变体”这个数据类型。但是，使用变体作为参数数据类型并不能真正实现泛型编程。在这一段，我们详细解释一下如何使用变体传递不同类型的数据，以及变体类型的局限。
...
多态 VI 虽然非常好用，但是制作过程还是相当麻烦的。它需要为每一种可能会用到的数据类型都编写一个 VI，更麻烦的情况是，某些算法允许用户使用各种自定义的数据类型，多态 VI 的开发着根本无法穷举所有可能出现的数据类型。因此，LabVIEW 还提供了其它的方式来让子 VI 支持多重数据类型的参数，比如“自适应 VI”。LabVIEW 有很多自带的自适应 VI，比如函数选板上，这种以深橘红色为底色的 VI，就多是自适应 VI。
...
与[内嵌子 VI](optimization_inline_subvi) 相似，当一个自适应 VI 被放置在程序框图上，它会把自己的代码嵌入到调用它的 VI 程序框图中去。不同之处在于：普通内嵌子 VI 的参数类型是固定的，就是它的前面板控件的数据类型；自适应 VI 前面板的控件更像是一个模板，它还会根据调用 VI 传递给它的数据的类型进行调整，任何它可以处理的数据类型都会被认作是合法的。这种机制与 C++ 在泛型编程中使用的模板相类似。

对比自适应 VI 与多态 VI，它们主要有以下差别在于：多态 VI 是一组预先写好的 VI，每个实例 VI 之间可以完全不相关，甚至参数个数都可以不同；自适应 VI 只是一个单个的 VI。由此可见，制作多态 VI 的工作量更大，同时它的功能也更强大，多态 VI 适合应对与编写功能复杂的工具包，提供给客户使用。自适应 VI 和普通 VI 之间可以灵活转换，又能灵活的支持多重数据类型，对于一个算法就可以应用到很多不同类型数据上的 VI，尤其适合被做成自适应 VI。

### 编写自适应 VI

这里使用一个 LabVIEW 自带的程序来演示如何编写和使用自适应 VI。假设我们需要编写一个洗牌的子 VI，把一副扑克牌传递给这个子 VI，这个 VI 会随机重现排列所有纸牌的顺序。这个洗牌的算法如下：
...
这个算法其实不但可以用来洗牌，而且也可以应用于随机打乱任意一组数据：无路是一组整数，一组字符串还是一组扑克牌，都是可以使用同样的洗牌算法。算法与具体数据类型无关。如果我们编写的子 VI 就只能用于扑克牌数据那就太可惜了。我们可以把这个 VI 设置为自适应 VI，这样就可以轻松的把它应用在任何类型的数据上了。

首先我们创建一个自适应 VI：

![images_2/z049.png](images_2/z049.png "创建自适应 VI")

我们也可以先创建一个普通 VI，再通过更改 VI 的文件后缀名把它变成自适应 VI。

接下来实现上面列出的洗牌算法逻辑，自适应 VI 也需要有输入输出控件，这里可以使用任何一种合法的数据类型：任意一种一维数组数据类型。这个数据类型会在自适应 VI 插入到调用 VI 的时候被替换。我们实现的示例中使用了最常见的数值数组：
...
上文编写的洗牌程序 shuffle_array.vim 只能处理一维数组，如果试图把其它类型的数据，比如二维数组，或者字符串数据传递给它，程序会报错，无法执行。

![images_2/z052.png](images_2/z052.png "自适应 VI 无法接受的数据类型")

这里的机制是，如果在自适应 VI 中使用某种类型的控件，VI 有错误，那么在调用自适应 VI 的程序中把这种类型的数据传递给自适应 VI 也会出错。我们可以尝试在上述示例的自适应 VI 中把输入输出控件改为字符串控件，VI 显示有错误：

![images_2/z053.png](images_2/z053.png "自适应 VI 不支持的输入控件类型")

如果希望这个自适应 VI 能够支持字符串数据，那么我们首先要意识到，shuffle_array.vim 的代码是不是用与字符串数据，我们需要为字符串数据定义一个新的功能或算法。假设，这里我们的需求是：如果输入一段字符串，自适应 VI 应当把字符串中的每个字符随机排序；如果输入数据是一维数组，则行为如上不变。定义好了这个行为，我们就可以实现一个新的自适应 VI 了：shuffle_string_and_array.vim。在这个新的自适应 VI 中，我们需要使用“类型专用结构”为自适应 VI 选择对应与不同输入数据类型的代码。
...
* 如果输入参数是一个一维数组，那么类型专用结构的 0 号分支，就会被采纳，因为 shuffle_array.vim 可以接收一维数组输出类型
...
虽然我们可以在自适应 VI 实现很多套代码，支持各种不同的数据类型和算法，但是自适应 VI 最适合的应用场合还是用于把同一个算法（同一段代码）应用于一些列不同而数据类型。加入多套代码很容易破坏代码的可读性。在需要多套代码的场合，使用多态 VI，或者编写一个类可能是更好的方式。

### 类型检查

在某些应用中，我们可能需要一个自适应 VI 只可以接收某种特别的数据类型，或者需要一个自适应 VI 只拒绝某一种特别的数据类型。这就需要用到各类类型对比函数和 VI 了。这其中，最为常用的是两个函数：
“Assert Structural Type Match”和“Assert Structural Type Mismatch”。它们各自有连个输入参数。如果两个输入数据类型相同，Assert Structural Type Match 不会有语法错误；如果两个输入数据类型不同，Assert Structural Type Match 会出现语法错误。Assert Structural Type Mismatch 则正好相反，在两个输入数据类型相同时出现语法错误。

比如，我们需要编写一个自适应 VI，它可以把两个输入的数据捆绑成一个簇，然后输出。但是有一个额外的要求，就是两个输入的数据的类型必须相同，比如同为整数，或同为字符串。一个布尔型，一个字符串这种需要报语法错误。LabVIEW 自带的捆绑成簇函数是不需要输入为相同数据类型的。所以不能仅仅一来它做类型检查，我们还需要在子适应 VI 中加入一个 Assert Structural Type Match 来查看两个输入数据的类型是否相同：
...
借助自适应 VI，我们已经可以满足泛型编程的大多数需求了。但是也有一些更复杂的需求是自适应 VI 也无法胜任的。首先自适应 VI 是一种[内嵌 VI](optimization_inline_subvi)，有些 LabVIEW 功能是无法在内嵌 VI 中使用的，比如某些[VI服务器](vi_server)的属性和方法无法使用。在内嵌 VI 中也无法对类的数据进行解除捆绑，即便自适应 VI 也在同一个类中也不行。

此外，一些逻辑更复杂的与类型相关的操作也无法使用自适应 VI 实现。比如，我们需要编写一个子 VI，它可以接收任意种类的簇类型的数据，对于接收到的簇数据，子 VI 会把簇中元素反向排列，然后输出新的簇数据。这样的需求目前还无法使用自适应 VI 来实现。


## 应用实例

我们在[面向对象的应用实例](oop_use_cases)一节的最后实现了一个双向链表容器。这个双向链表的实现有一个比较大的缺陷，就是它只能用于存储一种固定类型的数据。在示例中，我们用它来保存和处理实数数据。假设出现了一个新的需求，需要一个用于保存字符串的双向链表容器，那么我们只能在重现编写一套类似的类和 VI 来处理字符串数据，尽管双向链表的每个算法与保存的数据的类型都是无关的。

接下来我们把它改进一下，让它成为一个可以支持多种数据类型的泛型容器。用户可以使用同一套 VI 来创建和操作保存实数、字符串或其它数据类型的双向链表。

上文中我们提到的示例都是单个的泛型 VI，一个单个的 VI 可以支持多种数据类型。但是对于数据容器来说，我们需要实现一个泛型类或者泛型库，库中不同 VI 可以接收的数据不是独立的，它与其它 VI 接收的数据类型是相关的。比如这个泛型双向链表容器，它虽然可以支持多种数据类型，但是一旦实例化，用户创建了一个具体的链表，这个具体链表容器内就只能保存一种类型的数据。比如，用户创建了一个字符串链表，那么容器的每个插入数据方法，就应当确保插入的数据必须是字符串。

实现这个功能，我们需要一种可以把数据类型信息在 VI 之间传递的机制。LabVIEW 的数据线就具备这个功能，LabVIEW 的数据线不但传递数据，也同时传递数据类型的信息。然而遗憾的是，我们没办法使用 LvClass 数据线来传递所需数据类型，因为 LabVIEW 中的类是静态的，在运行前，类中的数据类型就确定了，不支持泛型类。但是我们还是可以使用其它数据线来达到这一目的，比如使用簇数据类型。

单从运行效率考虑，我们从头设计一个全新的泛型双向链表，但是因为我们已经实现了一个双向链表容器了，为了简化工作量，我们就在已实现好的代码上，进行改进。大致思路如下：把原有的双向链表容器类中的数据类型由实数类型，该换为变体类型，这样我们就可以把任何类型的数据都插入到这个容器里了。我们再使用自适应 VI 对原双向链表容器类中的方法进行包装，并加入数据类型检查功能。这个方法与上文提到的 Java 支持泛型编程的方式有相似之处。

### 使用变体保存数据

双向链表类的详细制作过程在[面向对象的应用实例](oop_use_cases)已经介绍过了，这里不在重复。我们需要改造的是把链表节点用于保存数据的控件类型改为变体类型：

![images_2/z060.png](images_2/z060.png "节点类的数据")

相应的，所有与数据读写相关的 VI 中对应的参数都需要改为变体类型，比如这是迭代器 Iterator 类中的读取数据的 VI：
...
在目前版本的 LabVIEW 中，泛型容器无法直接使用一个类来实现，因此，我们也不需要用 LvClass 来封装这个泛型链表的方法 VI 了。我们可以使用库（LvLibrary）来封装所有的方法。实现好的库类似下图：

![images_2/z062.png](images_2/z062.png "泛型链表项目结构")

这个泛型链表容器的所有方法都是自适应 VI，而不是普通 VI。

### 创建新链表方法

使用特定类型的链表容器时，不需要一个特别的初始化方法，因为没什么特别的参数需要设置。但是泛型链表容器必须要初始化一个参数，就是容器中保存的数据的类型。如果容器被设置为保存整数，那么之后再使用这个容器时就只能给它传递整数数据；类似的，如果容器被设置为保存字符串，那么之后再使用这个容器时就只能给它传递字符串数据。

我们实现了一个名为 new_list.vim 的自适应 VI，用于创建一个新的链表容器。它只需要一个输入参数“element data type”，用于表示新建链表中数据的类型。需要注意的是，这个控件中的数据是没有意义的，我们只需要用到它所代表的数据类型。链表中真正的数据还是被保存在我们之前已经改造好的双向链表类（DoublyLinkedList.lvclass）的实例中。所以，在这个创建链表方法中，我们还需要生成一个 DoublyLinkedList 的对象。DoublyLinkedList 的对象，和数据类型一起被捆绑成一个簇。其它的泛型链表容器的方法都需要使用到这个簇，这样那些方法 VI 才可以知道正在操作的是哪个容器以及容器中数据的类型是什么。
...
程序的 list in 输入参数，就是 new_list.vim 中创建出来的代表新的链表的输出数据。程序首先松绑 list in 数据得到 DoublyLinkedList 对象，和容器中数据的类型。接下来程序使用上文提到的 Assert Structural Type Match 函数，来比较需要保存的数据的类型是否与创建容器时指定的类型是否相同。如果数据类型不同，VI 会出现语法错误。如果相同，则调用 DoublyLinkedList 类中对应的 inster_before 方法，把新数据保存在 DoublyLinkedList 对象中。

### 读取数据的方法

因为保存在 DoublyLinkedList 类中的数据是变体类型，泛型容器的方法需要从 DoublyLinkedList 类中读取数据后再转换成特定的类型。下图是 to_array 方法，用于把链表中的所有数据读出，以数组的方式返回：
