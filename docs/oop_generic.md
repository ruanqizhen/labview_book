...
如果一个 VI，使用变体作为参数数据类型，这个 VI 的参数就可以接收任何类型的数据了，这样是不是就算作支持泛型编程了？仅仅只是能够接受不同数据类型是不够的，还要具有静态的类型检查功能才可以支持泛型编程。比如，某一算法可以支持数值和字符串类型的数据，但是不支持布尔类型数据，那么就需要实现这样的功能，当把布尔数据类型于这个算法子 VI 的输入连线的时候，VI 就会立刻报错，不需要运行，VI 就可以提示数据类型出错。 子 VI 使用变体数据类型显然无法做到这一点。甚至更复杂的情况，如果创建了一个字符串类型的数组，再使用其它数组函数（比如插入、索引等）对这个数组操作时，函数应当也只能接收字符串类型的数据，连接到其它数据类型也需要报错。
...
单从运行效率考虑，我们从头设计一个全新的泛型双向链表，但是因为我们已经实现了一个双向链表容器了，为了简化工作量，我们就在已实现好的代码上，进行改进。大致思路如下：把原有的双向链表容器类中的数据类型由实数类型，该换为变体类型，这样我们就可以把任何类型的数据都插入到这个容器里了。我们再使用自适应 VI 对原双向链表容器类中的方法进行包装，并加入数据类型检查功能。
