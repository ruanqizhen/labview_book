...
## 基础知识
我们在编写程序的时候总是要把一些通用的功能抽象出来。比如说，我们设计了一个排序的算法，我们一般不会直接在每个需要排序的地方都针对特定的数据重复为同一个算法编写代码。我们通常会把这个算法抽象成一个子函数（子 VI），把需要被排序的数据作为函数的参数。因为被抽象出来的算法是与具体数据无关的，所以同一个算法可以被应用到不同的数据上。比如，同一个排序算法既可以对数组 `[1,3,2]` 排序，也可以对组数 `[5,6,4]` 排序。
...
之前，我们创建子 VI，输入输出的数据类型（控件类型），一旦选定，就固定下来的。比如用一个数值控件表示输入数据的子 VI，就只能传递数值数据给它。把字符串数据线连接上，会出现语法错误。要在 LabVIEW 中使用泛型编程，我们需要编写一种子 VI，让它的某个参数可以接受不同类型的数据。大家可能已经注意到了，LabVIEW 自带的很多函数是支持泛型的，比如加法函数，可以接收数值类型的数据，也的泛型编程既可以及支持数可以支持多种数据类型")

如果一个 VI，使用变体作为参数数据类型，这个 VI 的参数就可以接收任何类型的数据了，这样是不是就算支持泛型编程了？（或者是像 Python 2.0 这样的编程语言，变量不需要有类型定义，任何类型的数据都可以传递给同一个变量，是不是这样就算支持泛型编程了？）
...
一种是以 Java 为代表的类型擦除技术（erasure）。当编译器对带有泛型的 Java 代码进行编译时，会执行数据类型检查和类型推断，如果遇到类型错误会报错。但是编译器生成的执行代码是不带泛型的代码，也就是数据类型信息被擦除了，因此同样一段代码可以被 Java 虚拟机用来处理各种不同类型的数据。这样，Java 的泛型编程既可以支持多种数据类型，又可以保证类型安全。

比如，在代码中定义了 `List<Integer>` 或 `List<String>` 等类型。在代码编译时，编译器会检查传递的数据是否是 Integer 或 String 等类型，在代码编译后，它们就都会变成 List。Java 虚拟机看到的只是 List，于是相关的函数不论 List 中数据是何类型，都能正常运行。

另一种是以 C++ 为代表的模板实例化（Template Instantiation）技术，也被称为代码插入技术或者代码膨胀技术。在 C++ 中编写泛型函数或者泛型类的时候，其实并非直接编写一个特定的函数或类，而是编写了一个函数或类的模板。比如需要一个支持泛型的 List，那么就要编写一个 List 的模板。程序中使用到了 `List<int>` 或 `List<string>` 等类型的地方，编译器会自动从模板生成对应特定数据类型的底层代码。`List<int>` 或 `List<string>` 生成的是不同的代码，在编译程序时，编译器就会检查代码中是否有数据类型错误，确保类型安全性。由于程序中有大量从模板自动生成的代码，代码空间会比较大。

在 LabVIEW 中，我们主要通过三种方式来实现泛型编程：传统的变体 (Variant) 传递、多态 VI (Polymorphic VIs)，以及最接近现代泛型概念的 自适应 VI (Malleable VIs, .vim)。这一节将逐一介绍它们的原理和局限性。
...
尽管使用变体属性实现的的查询效率极高，但它毕竟不是一个真正的数据容器，还有一些局限性，比如它的标识只能使用字符串。在 LabVIEW 有了 Map 数据类型之后，就没必要再使用变体来保存查询数据了。
...
自适应 VI 的机制与 C++ 的模板 (Template) 非常相似。默认情况下，LabVIEW 会将自适应 VI 设置为 “内嵌 (Inlined)”，这意味着它会把代码直接铺展到调用者的程序框图里。
...
但是，自适应 VI 并不是必须内嵌的。如果你的 VIM 内部包含了不支持内嵌的代码（例如递归调用、某些属性节点），你可以取消勾选“内嵌”属性。此时，当你在主程序连上一根特定类型（如字符串）的线时，LabVIEW 编译器会在后台偷偷为你生成一个专属于“字符串”的独立子 VI 实例，并进行调用。

自适应 VI 前面板的控件更像是一个占位符（Placeholder），在编译时，LabVIEW 会自动检查传入的数据类型是否能让 VIM 内部的代码“走得通”（没有语法断线）。如果走得通，编译成功；如果走不通，就报错。
...
类型专用结构是一个专用于自适应 VI 的核心结构。它可以包括多个分支。LabVIEW 在编译时，会按照分支的顺序从前至后进行验证。如果某个分支因为传入的数据类型不兼容（导致连线断裂），LabVIEW 会静默忽略这个分支，继续尝试下一个分支。直到找到第一个连线完全合法（类型匹配）的分支，就会采纳该分支的代码进行编译。（注意：如果是缺失子 VI 或非类型相关的语法错误，编译器依然会报错）。这样，我们就可以针对不同的输入类型（如数组、字符串、标量），在各个分支里编写定制的处理逻辑。
...
如果我们从零开始，设计一个全新的泛型双向链表，会有比较好的运行效率。但是因为我们之前已经实现了一个双向链表容器类了，为了简化工作，我们就不再重新设计了，而是把已有的类做微调，再包装一下。大致思路如下：把原有的双向链表容器类中的数据类型由实数类型，更换为变体类型，这样我们就可以把任何类型的数据都插入到这个容器里了，实现了运行时支持任意数据类型。再使用自适应 VI 对原双向链表容器类中的方法进行包装，并加入编译时的数据类型检查功能。这个实现泛型编程的方法与上文提到的 Java 支持泛型编程的方式有相似之处。
...
使用同样的一套自适应 VI，我们既可以创建一个用于保存实数的链表，也可以创建一个用于保存字符串，或者任何其类型数据的泛型双向链表。
