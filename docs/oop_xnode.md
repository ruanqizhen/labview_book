...
到目前为止，我们制作的 XNode 仅仅只有个图标，并不能做任何事情。如果我们要给它添加点功能，像一个真正的函数或者子 VI 那样，我们需要先给它添加一些输入输出参数，也就是接线端。这需要通过 GetTerms4 功能 VI 来实现。

![images_2/z087.png](images_2/z087.png "GetTerms4")

这个功能 VI 的核心输出是一个数组，数组的每个元素是一个簇。每个簇包含了一个接线端的数据，它们分别是：
