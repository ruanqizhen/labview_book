...
![](images/image552.png "在不同的运行方式下，得到不同的路径")
...
在把程序生成为可执行文件 (EXE) 后，VI 作为独立文件不再存在，而是被打包进 EXE 文件中。如果你在程序中使用了“当前 VI 路径 (Current VI's Path)”常量，它在开发环境下会返回 C:\MyApp\Main.vi，而在生成的 EXE 中会返回 C:\MyApp\App.exe\Main.vi（多出了一层虚拟的 exe 目录）。这会导致相对路径计算错误，是初学者最容易踩的坑。

在“编程 -> 文件 I/O -> 文件常量”选板中，LabVIEW 提供了一个专门的 “应用程序目录” (Application Directory) VI。

该 VI 非常智能：
- 在开发环境下运行，它会自动返回当前所处的 .lvproj 项目文件所在的目录（或顶层 VI 所在的目录）。
- 在生成的 EXE 环境下运行，它会自动返回 EXE 文件所在的真实物理目录。

因此，你只需要将“应用程序目录”与你的相对路径（如 data.ini）使用“创建路径”函数拼接即可，代码无需做任何开发/发布状态的条件判断，完全无缝兼容。
...
### 使用 VIPM (VI Package Manager)

目前 LabVIEW 整个生态环境（包括 NI 官方的工具包）都在使用 JKI 公司开发的 VIPM 来管理和发布源代码。

使用 VIPM，可以将写好的子 VI 打包成一个后缀为 .vip 的安装包。在打包过程中，可以直观地配置这些 VI 在函数选板上的具体位置、添加许可协议、管理依赖的其他工具包。用户只需双击 .vip 文件，VIPM 就会自动处理底层的所有选板配置和文件释放，一键安装卸载，这是发布 LabVIEW 源码库最专业的方式。
...
原有基于“Web 发布工具”生成的 HTML 页面，依赖于用户浏览器安装“LabVIEW Web Player”插件（基于旧式的 NPAPI 技术）。目前，包括 Chrome、Edge、Firefox 在内的所有现代主流浏览器已经彻底停止支持该插件。因此，传统的浏览器端远程前面板技术已经基本被淘汰，仅能在独立的 LabVIEW 运行引擎客户端中作为独立窗口打开。

现代 Web 解决方案：
如果需要让用户通过手机或现代浏览器控制 LabVIEW 程序，目前业界标准的做法有两种：
- LabVIEW Web Services + 前端技术： 使用 LabVIEW 编写 Web Services 提供标准的 RESTful API（返回 JSON 或 XML 数据），然后由专业的前端开发人员使用 HTML5/JavaScript（如 Vue, React）编写纯网页前端进行交互。
- G Web Development Software (原 WebVIs)： 也就是 NI 官方提供的现代 Web 模块，它允许你使用图形化编程（类似写 VI）来生成纯正的、无需任何插件的 HTML5/JS 网页，从而与后台的 Web Services 完美对接。
...
在程序生成规范的菜单中选择“新建 -\>web 服务”，在属性对话框的源文件页，可配置生成服务 VI。
...
![](images/image570.png "查看 Packed Project Library 中的内容")
...
打包库 (.lvlibp) 在实现插件化架构时非常有用，但它也有一个常常让架构师头疼的致命约束：极强的版本与平台依赖性。
- 严格的 LabVIEW 版本绑定： 打包库是已编译的二进制文件。如果你用 LabVIEW 2022 编译了一个 PPL，调用它的主程序（或另一个 PPL）必须也是在 LabVIEW 2022 下运行。它不能像普通源代码 VI 那样被高版本的 LabVIEW 自动升级重编译。
- 严格的平台与位数绑定： 为 32 位 Windows 编译的 PPL，绝不能在 64 位 LabVIEW 或 Linux RT 系统中调用。你需要为部署的每一个目标平台单独编译一份专属的 PPL 文件。

因此，如果代码需要广泛提供给使用不同版本 LabVIEW 的开发者，请发布源代码或 VIPM 格式；如果代码仅用于公司内部特定版本项目的组件化集成，那么使用打包库 (PPL) 会极大缩短编译时间。
