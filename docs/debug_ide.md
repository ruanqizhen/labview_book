...
与设值断点类似，使用工具选板上的探针工具，或者直接在程序框图的数据线上右击鼠标，选择探针菜单项，可以为数据线添加一个探针。LabVIEW 的探针也是图形化显示的，数据被显示在悬浮窗显示区域的显示控件中。探针窗口有左右两部分，左边列出了当前程序中所有的探针，高亮选中其中的一个探针，它的数值会被显示在窗口右边“Probe
...
我们可以根据自己的需求，修改其外观，但是在示例中，我们仅仅需要显示队列中数据的值，这个外观已经可以胜任了，所以我们不需要做任何改动。这个框架 VI 的程序框图也是改编自数组的探针，它无法直接处理队列数据，所以 VI 是无法运行的：
...
LabVIEW 默认使用一个集成的“探针监视窗口”来管理所有探针。

这个窗口的左侧是一个列表，列出了所有已创建的探针。对于简单的数值、布尔或简短字符串，我们可以直接在列表中同时看到所有探针的当前值，这对于监视多个变量非常方便。

只有当某个探针的数据类型比较复杂（如大数组、复杂的簇或波形数据）时，我们才需要在列表中选中它，然后在窗口右侧的详细显示区查看其具体内容。如果确实需要同时详细监视多个复杂的数组，可以使用窗口左上角的“在新窗口中打开”按钮，将特定的探针分离成独立窗口显示。

![](images/image503.png "在独立窗口显示探针数据")

在旧版本（LabVIEW 2009 之前）中，是没有集成的“探针监视窗口”的，每个探针都有一个独立窗口的。
...
如果一个项目需要支持多个操作系统，我们当然可以为每个操作系统都编写一套包含所有子 VI 的完整的程序，但采用这种方式，程序中每个 VI 都需要同时维护多个版本，尽管很多 VI 在每个操作系统上都是一模一样的，这极大增加了维护工作量。使用条件禁用结构，我们可以把针对不同操作系统的代码分别写在条件禁用结构的不同分支内，LabVIEW 会根据 VI 运行的系统选择正确的分支运行。这样，一个程序就可以适应所有的操作系统，最大可能的降低了维护成本。
...
配置条件框上的“符号”可以是系统与定义好的一些符号。最常用的用于判断操作系统的符号是 OS。

OS 的值对应当前的操作系统平台。例如，如果配置 `OS == Windows`，则该分支在 Windows 系统下启用；如果配置 `OS == Linux`，则在 Linux 系统下启用；Mac 系统则对应 `OS == Mac`。

LabVIEW 还有其他符号，比如 TARGET_TYPE，TARGET_BITNESS 和 RUN_TIME_ENGINE。“值”则是“符号”对应的数值。配置条件框配置的是个条件，可以选相等或不相等。旧版本的 LabVIEW 可能没有 OS 这个系统预定的符号，在那些版本上，可以使用 TARGET_TYPE 表示当前 VI 运行的操作系统，大多数读者使用的是 Windows 操作系统,如果配置条件框为条件禁用结构的某一分支配置了“TARGET_TYPE == Windows”，那么这个分支的条件就满足了，这个分支就会变成启用状态。笔者使用的操作系统是 Linux，LabVIEW 中对应 Linux 的 TARGET_TYPE 的值是 Unix，所以在笔者的电脑上，这一个分支是被启用的：

![images_2/z128.png](images_2/z128.png "配置条件框")

TARGET_BITNESS 用于判断计算机上安装的是 32 bit 还是 64 bit 的 LabVIEW。

RUN_TIME_ENGINE 则用于区分程序的运行环境：
- False：表示当前 VI 正在 LabVIEW 开发环境（Development System）中运行。调试代码时通常处于此状态。
- True：表示当前 VI 正在 LabVIEW 运行引擎（Run-Time Engine）中脱离开发环境运行。这意味着该 VI 已经被编译成了可执行文件（EXE）或共享库（DLL/Shared Library）。

这个符号常用于区分开发阶段和最终部署阶段的行为，例如在开发时不检查加密狗，而在生成 EXE 后启用加密狗检查。
