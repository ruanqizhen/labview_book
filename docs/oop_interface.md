...
由于接口中定义的方法，默认状态下是要求必须在类中重写的，如果某个类没有重写接口中定义的方法，LabVIEW 会报错。新添加的类可能会缺少很多方法，这时候可以在选择 “新建 -> 用于重写的 VI” 的时候，一次选中所有缺失的 VI，然后点击生成。

![新建用于重写的 VI 时可以多选](images_2/image65.png "新建用于重写的 VI 时可以多选")
...
![测试程序的程序框图](images_2/image64.png "测试程序的程序框图")
...
![两个类分别实现了两个同样的接口](images_2/image66.png "两个类分别实现了两个同样的接口")

类 CBar 和类 CFoo 各自都继承了接口 IAAA 和接口 IBBB。这时候，把一个 CBar 的对象和一个 CFoo 的对象绑定成一个数组，程序会出错，提示有类型冲突：

![两个有多个共同接口的类绑定成数组会出错](images_2/image67.png "两个有多个共同接口的类绑定成数组会出错")

遇到这种情况，就只能程序员手动指定一个需要使用的接口类型了：

![手动指定接口类型](images_2/image68.png "手动指定接口类型")
...
![从接口生成的实例](images_2/image69.png "从接口生成的实例")

好的接口设计应该可以尽量防止程序员错误的使用接口，所以我们需要对这种情况做一些预防措施。我们可以将接口中的动态分派 VI 勾选为 “必须在后代类中重写 (Require descendant classes to override this VI)”，LabVIEW 的编译器就会接管防错工作。如果有人试图用一个孤立的接口常量直接去调用这个 VI，连线会直接断开，程序根本无法点击运行按钮！编译器会强制要求流入该节点的必须是一个实现了该接口的具体子类对象。因此，接口的抽象性在 LabVIEW 中是有严格的编译级保障的。

因为在接口中定义的基于动态分配模板的 VI 是必须在子孙类中被重写的，所以接口中的 VI 的代码在正常情况下，永远都不应该被执行到。如果被执行到了，说明这个方法是被一个从接口直接生成的对象调用的。如果这时候程序报错，就可以提醒编程者及时修改这个错误。所以，接口中的方法，虽然应该不会被执行到，最好也不要空放着，而是应该返回一个错误信息以避免潜在的问题。下图是一个接口中方法 VI 的程序框图：

![如果接口中的 VI 被调用，则返回错误](images_2/image70.png "如果接口中的 VI 被调用，则返回错误")

造成 LabVIEW 中没有抽象类的本质原因是因为，LabVIEW 中没有可以脱离数据的数据类型定义。LabVIEW 程序中必须使用一个数据来表示数据类型。在很多编程语言中，都有表示数据类型的关键字，比如我们可以使用关键字 “int” 来表示整数数据类型，但是 LabVIEW 中没有类似的关键字（或者常量、控件、节点等）。LabVIEW 中必须使用一个具体的数值，比如 0 或 23 等，来表示整数数据类型。回顾我们在 [数值数据](data_number) 一节介绍过的把实数强制转换为整数的例子：
...
因为 LabVIEW 的接口存在的代码复用的问题，我们在设计程序时，也需要考虑不能太过滥用接口，不要为了多继承而牺牲代码复用。接口的强项在于规范“行为（Can-do）”，而类的强项在于复用“状态与核心逻辑（Is-a）”。在 LabVIEW 架构设计中，“抽象类 + 接口”的混合模式才是最佳实践。

针对“合体桌椅”的需求，更好的架构应该是：
1. 保留 Furniture 抽象类： 它包含 ID 和 Cost 数据，并实现了获取价格的通用代码。
2. 定义接口： 定义 ITable 接口（规定铺桌布动作）和 IChair 接口（规定放靠枕动作）。
3. 单继承 + 多实现：
   - 普通桌子类：继承 Furniture 类，实现 ITable 接口。
   - 普通椅子类：继承 Furniture 类，实现 IChair 接口。
   - 合体桌椅类： 继承 Furniture 类（免费获得了价格和ID的代码复用），同时实现 ITable 和 IChair 接口。

通过这种架构，我们既享受了类继承带来的零重复代码红利，又利用接口完美解决了多重身份（多态）的调用冲突。
