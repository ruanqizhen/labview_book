...
在LabVIEW中，接口与类非常相似，主要区别只有两点：1.接口中没有数据，2.接口可以被多继承。LabVIEW的接口提供的接口最主要的功能：定义一个类可以被外部访问到的方法。但是对于另外两点，确保定义的抽象性和减少重复代码的支持非常有限。所以我们在程序中使用接口是，可能还需要额外做一些设计以强化接口的各方面功能。
