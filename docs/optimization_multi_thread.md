...
在理想情况下，操作系统会将这两个计算繁重的线程调度到不同的物理核心上，从而使双核 CPU 的占用率接近 100%。

真正的并行执行前提是这两个循环之间没有资源争夺。如果它们调用了同一个不可重入（Non-Reentrant）的子 VI，或者频繁访问同一个被保护的全局资源（如文件 I/O 或硬件驱动），那么它们实际上会退化为串行执行，CPU 占用率将不会提升，甚至因为线程切换开销而下降。后续章节中，会详细讨论这些特殊情况。
...
LabVIEW 的这样一套把 VI 代码调度、运行起来的机制叫做执行系统。新版本的 LabVIEW 有六个执行系统，分别是：用户界面执行系统、标准执行系统、仪器 I/O 执行系统、数据采集执行系统、以及其它 1、其它 2 系统。一个应用程序中使用到的众多子 VI 是可以分别放在不同的执行系统里运行的。用户可以在 VI 属性面板上的“执行”页面，指定这个 VI 的执行系统：
...
在 VI 属性面板的“执行”页面上，用户除了可以设置 VI 的执行系统，还可以设置它的优先级。优先级分为后台、标准、高于标准、高、实时等五个等级，优先级依次提高。优先级内还有一项“子程序”，它是一个比较特殊的设置。实际上它并不是一个优先级。
...
![](images_2/z358.png "VI 属性的“执行”设置页")
...
以上的三点保证了“子程序”VI 在执行时会得到最多的 CPU 资源。某些非常关键的运算，假如不是特别耗时的话，就可以被设置为“子程序”以提高运行速度。比如，有这样一个 VI，它的输入是一个数值数组，输出是这组数据的平均值。这个运算在程序中需要被尽快地完成，以免拖延数据的显示。这个 VI 就是一个非常适合“子程序”优先级的 VI。

在设置 VI 优先级时，需要注意如下几点：

提高一个 VI 的优先级并不能缩短 VI 总体占用 CPU 的时间。提高了 VI 的优先级，意味着它可以比其它 VI 更快地抢到 CPU 的资源，但它所需要的 CPU 时间还是那么多。
...
随意调整 VI 优先级可能会引发“优先级反转”问题。例如，一个“高优先级”的 VI 想要调用一个子 VI，但该子 VI 已经被一个“低优先级”的线程占用（且该子 VI 不可重入）。

此时，高优先级线程被迫等待低优先级线程运行完毕。如果此时还有一个“中优先级”线程在疯狂抢占 CPU，低优先级线程就一直运行不完，导致高优先级线程被无限期挂起，系统出现严重的卡顿。

最佳实践： 除非你非常清楚自己在做什么，否则尽量保持所有 VI 为“标准”优先级。解决性能问题的正道是优化算法和架构，而不是调整线程优先级。
...
后来，还是从同事提供的一些信息中得到了启发，才突然想明白了原因所在。程序耗用了大量的 CPU 资源，以至于动画刷新功能得不到足够的资源，是由于几乎全部的 CPU 资源都被消耗在线程切换中了。程序为每个 OpenGL 接口函数包装一个接口 VI。这些接口 VI 非常简单，程序框图就只有一个 CLN 节点，调用相应的 OpenGL 函数。每个 VI 都是在默认的执行线程中运行的，而 CLN 调用的函数却是在界面线程下运行的。因此，每执行一次这样的接口 VI，LabVIEW 都要进行两次线程的切换。先从执行线程切换到界面线程，执行完函数，再切换回执行线程。
...
摩尔定律曾预言，CPU 的主频每隔一年半左右就会提高一倍。但是这个定律在主频这个参数上已经失效了。笔者多年来换过的几台电脑，它们的 CPU 主频始终在 2GHz 到 4GHz 左右。目前 CPU 厂商对于提升 CPU 整体性能的最主要手段是增加 CPU 的内核数量。多个 CPU 内核同时工作，固然可以提高程序运行效率。但是，只有针对多核系统做了优化的软件，才能充分发挥多核的威力。针对多核进行优化的第一步，是要求软件是多线程运行的。在多核计算机上，操作系统会自动把程序的不同线程分配到不同的 CPU 内核上去运行。
...
总结一下，不能并行化的场景：
- 数据依赖： 如移位寄存器或反馈节点。
- 硬件资源限制： 如果循环内包含硬件 I/O 操作（如读串口、采集卡），强制并行化不仅不会加速，反而可能因为争夺硬件锁导致报错。
- UI 操作： 包含属性节点或调用节点的循环通常无法有效并行化，因为它们受限于单线程的 UI 系统。
...
标准的反馈节点是同步的：每次循环迭代严格产生一个输出，作为下一次的一个输入。1 对 1。

通道线反馈是异步的：在一次循环迭代中，你可以向反馈通道写入 N 个数据，或者不写入。下一次迭代读取时，可以读出这 N 个数据进行处理。这种N 对 M 的灵活数据传递是标准反馈节点做不到的。
...
虽然从功能上通道线看起来像“隐形的队列”，但其底层实现远比队列复杂。LabVIEW 编译器在后台会自动生成大量的辅助代码来管理数据传输和生命周期。

- 优点： 极大地简化了代码连线，不需要显式地创建和销毁引用，不需要传递引用句柄，非常适合快速原型开发。

- 缺点： 调试困难。通道线是一根“黑盒”管子。你无法在通道线上放置探针（Probe）来查看流过的数据，也难以监控缓冲区是否溢出。一旦出现数据堵塞或丢失，排查难度远高于标准的队列模型。

在简单的、非关键任务的数据传输中使用通道线。在构建大型、高可靠性、需要精细控制（如清空队列、预览元素、动态调整大小）的架构时，标准的队列（Queue）依然是首选。
