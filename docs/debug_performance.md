...
我们需要重点关注以下两列数据：
1. VI 时间（VI Time）： 仅代表该 VI 自身面板和框图代码消耗的时间，不包含它调用的子 VI 的时间。如果这项很高，说明该 VI 自身的算法或逻辑需要优化。
2. 子 VI 时间（Sub VIs Time）： 代表该 VI 及其所有子 VI 的总耗时。如果这项很高但“VI 时间”很低，说明性能瓶颈在它调用的底层子 VI 中，你需要双击该条目深入查看。
...
更简单一点，仅使用一个时间顺序结构也能完成同样的工作。比如下图示例中的“帧持续时间”（Frame Duration）输出与上图中的 x-y 含义相同。

![](images/image521.jpeg "使用时间顺序结构查看程序运行时间")

传统的 Tick Count (ms) 函数受限于操作系统的时钟中断频率，精度通常在 1ms 到 15ms 之间，无法测量极短代码的耗时。对于需要微秒级甚至更高精度的基准测试，应使用 “高分辨率相对秒” (High Resolution Relative Seconds) VI。该 VI 利用 CPU 的高精度硬件计时器，能够准确测量极短的执行时间。

此外，为了获得准确结果，通常建议将待测代码在循环中运行多次（例如 1000 次），计算总时间后取平均值，以消除系统抖动的影响。

使用时间结构时需要注意，时间结构可能会改变其内部程序运行的线程。这样一来，使用它，可能会影响程序运行的时间。
...
树形控件确实比较消耗资源。除了上文提到的通过属性一次性写入外，LabVIEW 还提供了一个专用的 调用节点 (Invoke Node) 方法："Add Multiple Items"。相比于直接设置属性，使用 "Add Multiple Items" 方法通常能获得更好的性能，尤其是当需要处理成千上万条数据时，它能更好地管理内存并减少重绘开销。
...
除了降低刷新率，必须极力避免在循环中频繁调用属性节点 (Property Node)（如改变控件的颜色、位置、可见性等）。

LabVIEW 的属性节点必须在 UI 线程 中执行，而普通运算代码在 执行线程 中。每次在循环中调用属性节点，CPU 都必须暂停当前运算，切换到 UI 线程更新界面，然后再切回来。这种频繁的“线程切换”是极其昂贵的。

优化策略：
- 尽量将属性节点的设置移到循环外。
- 如果必须在循环中更新，请使用局部变量（Local Variable）代替“值”属性节点（Value Property），因为局部变量不强制切换到 UI 线程。
- 检查控件的高级属性，确保 “同步显示 (Synchronous Display)” 选项被关闭（默认是关闭的，但有时会被误开），否则每次更新都会强制 CPU 等待显卡绘制完成。
...
将局部变量移出循环确实能提高速度，但要小心逻辑改变。
- 场景 A（外部控制）： 如果该变量是用来接收外部按钮指令的，移出循环会导致程序无法响应操作。此时应考虑使用“队列”或“通知器”来替代低效的轮询变量。
- 场景 B（数据迭代）： 如果变量只是为了在循环的下一次迭代中使用上一次的值（如累加计算），绝对不要使用局部变量。

移位寄存器直接在寄存器或高速缓存中传递数据，没有任何数据拷贝和线程同步的开销，是 LabVIEW 中处理循环迭代数据的最高效方式，比局部变量快得多。
