...
动态链接库的英文是：Dynamic Link Library，简称 DLL。从字面上理解，它是一种“程序库”。库内存放的是可供应用程序使用的函数、变量等。
...
DLL 的使用非常普遍。比如 Windows 操作系统提供给应用程序调用的功能，就是以 DLL 的形式公布出来的。LabVIEW 中若需使用某个系统功能，如读写注册表等，即可通过调用 Windows 提供的 DLL 函数来完成。Windows 提供的这些完成系统功能的函数也被称为 Windows API。在常用的几个系统 DLL 中，kernel32.dll 提供了内存管理和进程调度相关的函数，user32.dll 中的函数则主要用于控制用户界面，gdi32.dll 中的函数则负责图形方面的操作。

需要特别注意的是 DLL 的路径与位宽（Bit-ness）： LabVIEW 本身分 32 位和 64 位版本。32 位的 LabVIEW 只能调用 32 位的 DLL，64 位的 LabVIEW 只能调用 64 位的 DLL，两者互不兼容。

在 32 位 Windows 系统中，DLL 确实位于 `C:\Windows\System32`。

但在目前主流的 64 位 Windows 系统中，微软的命名规则具有迷惑性：64 位的 DLL 存放在 `C:\Windows\System32` 目录下，而 32 位的 DLL 实际上存放在 `C:\Windows\SysWOW64` 目录下。因此，如果你的 LabVIEW 是 32 位的，调用系统 API 时实际上用到的是 SysWOW64 目录下的文件。
...
需要注意的是，如果 DLL 使用了 C++ 的类作为接口，这样的 DLL 无法在 LabVIEW 中直接调用。CLN 节点只能调用符合标准 C 语言函数接口的 DLL。若项目中必须使用某个 C++
DLL 时，可以在其上再用 C 语言写一个 C 接口 DLL，作为它和 LabVIEW 之间的中间层。LabVIEW 调用这个中间层 DLL 提供的函数，中间层函数再调用 C++ 接口的 DLL 函数。

## DLL 的加载方式

一个 CLN 节点刚被拖到程序框图上时，外观如下图所示：
...
还有一种情况，使用动态加载的优势比较明显。假如程序功能很多，调用了多个 DLL 文件。而程序每次运行时，往往只用到其中部分 DLL。如果使用静态加载，不管有没有用到，所有的 DLL 都要被装入内存。如果此时缺失了某个 DLL，即便程序运行时不需要用到它，程序也会因为在启动时找不到它而无法运行。采用动态加载方式，只是把那些程序运行时用到的 DLL 装进来。这样，提高了程序效率，也不会因为缺少某个暂时用不到的 DLL 而影响程序运行。
...
可以按照以下判断方法，选择 CLN 节点中的线程设置：

1. 默认规则： 如果不确定，或者 DLL 文档未明确声明“Reentrant”（可重入）或“Thread Safe”（线程安全），请务必选择“在 UI 线程中运行”。
2. 涉及界面： 如果 DLL 中的函数会弹出窗口、对话框，或者操作 ActiveX 控件，必须选择“在 UI 线程中运行”，否则可能导致 LabVIEW 界面假死。
3. 追求效率： 只有明确知道该算法是纯数学计算且支持多线程并发时，才选择“在任一线程中运行”。

如果被调用的动态链接库是多线程安全的，就选择“在任一线程中运行”；否则，动态链接库就不是多线程安全的，就得选择“在 UI 线程中运行”。选择在任一线程中运行一个 DLL 函数，程序的运行效率比较高。因为 LabVIEW 可以把 DLL 函数放在与其前后程序代码相同的线程内执行，这样就省去了线程切换的开销。并且，该设置允许 LabVIEW 在不同的线程内同时调用同一个 DLL 函数，并行执行的速度通常比串行高一些。但是，如果 DLL 不是多线程安全的，也就意味着在不同线程内同时调用 DLL 中的函数可能会出现错误，那么必须禁止这种情况的出现。这时，只能把 CLN 节点设置为“在 UI 线程中运行”，以确保所有 DLL 函数都只在一个线程内运行。

判断一个动态链接库是否是线程安全的，也需要费一番心思。如果这个动态链接库文档中没有明确说明它是多线程安全的，那么，为稳妥起见，应该把它当成非多线程安全的。熟悉 C 语言的用户可以查看一下动态链接库的源代码，若代码中存在全局变量、静态变量或者代码中看不到有使用信号量，关键区等保护措施的，这个动态链接库也肯定不是多线程安全的。
...
C 语言中经常在函数间传递指针或者数据的地址。在 32 位的程序中，可以使用 int32 数值来表示指针。因此，当需要在 LabVIEW 中传递指针数据时，可以使用 I32 或 U32 数值类型来表示这个地址类型的数据。但是，在 64 位的程序中，数据的地址只能使用 I64 或 U64 来表示。这样在一个调用了 DLL 函数、并且函数参数中有地址型数据的 VI，如果使用固定数据类型的数值来表示地址的话，就要准备两份代码。解决此问题的方法就是在该页的“数据类型”栏选择使用 LabVIEW 中的新数据类型：有符号或无符号 “指针大小整型”。这个数据类型的长度在不同的平台上会自动使用 32 位或 64 位长度。
...
显然，元素 a 只占用一个字节，而元素 b 占用四个字节。假设结构中的元素 a 所在的地址是 0xAAAA0000，那么，元素 b 占用四个字节的存放地址是与结构的字节对齐（Byte Alignment）设置相关的。如果采用 1 字节对齐，则 b 是紧挨着 a 存放的，b 的地址就是：0xAAAA0001；如果采用 2 字节对齐，b 的存放地址是紧挨着 a 的第一个偶数地址，也就是：0xAAAA0002；在 4 字节对齐模式下，char a 虽然只占 1 个字节，但为了让后面的 int b 能够放在 4 的整数倍地址上，编译器会在 a 后面填充 3 个字节的无意义数据（Padding）。LabVIEW 的簇（Cluster）在传递给 CLN 节点时，如果设置为“适配类型”，LabVIEW 会尝试匹配 C 的结构，但这种匹配往往是不稳定的。
...
强烈建议在编写 C 语言 DLL 时，使用 #pragma pack(1) 指令强制结构体按 1 字节对齐（即没有填充字节），这样 LabVIEW 的簇就能严丝合缝地对应上，无需手动添加占位用的字节。
...
为何函数返回值就不需要开辟存储空间了呢？以函数返回字符串为例，实际上函数返回的是一个内存地址（指针）。LabVIEW 拿到这个地址后，会读取其中的字符并复制一份到 LabVIEW 自己的字符串数据区中。

这里存在一个极易被忽视的风险：谁负责释放 DLL 返回的指针指向的内存？

- 情况 A：如果 DLL 返回的是静态常量字符串（如 const char* s = "Hello";），则是安全的。
- 情况 B：如果 DLL 内部使用 malloc 或 new 分配了内存并返回指针，LabVIEW 只负责读取，不会负责释放这块 C 语言内存。这意味着每调用一次该函数，就会发生一次内存泄漏。

针对情况 B，我们通常需要该 DLL 提供一个配套的 FreeString(char* ptr) 函数，在 LabVIEW 读取完返回值后，再调用这个释放函数来清理内存。
...
绝大多数时候调用 DLL 都是为了利用别人已经实现好了的功能。只有少数情况下，我们可能需要为某个 LabVIEW 项目专门制作一个 DLL 文件。比如，可能某些算法只有用 C 语言实现才能达到要求的效率；或者需要把某个无法直接被 LabVIEW 调用的外部模块再包装一层，等。对于这种专为 LabVIEW 调用设计的 DLL 文件有一些方法可以简化制作难度，以及提高运行效率。
