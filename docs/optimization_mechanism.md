...
如果在 LabVIEW 中按照与 C 语言完全相同的逻辑来实现此功能，LabVIEW 的程序框图将出现多层嵌套。在文本语言中，即便有多层条件嵌套，但所有代码都是平铺开的。程序员只需拖动鼠标，就可以查看全部程序代码。而在 LabVIEW 中，每次只能显示条件结构一个条件分支的代码。如果程序员，想要查看条件 "condition
...
在某些特殊情况下，一个 .vi 文件也可能保存了若干个子 VI（子函数）。其中，某些 VI 可能没有他们自己的 .vi 文件。这些 VI 没有前面板，而它们的程序框图就保存在调用它们的高层 VI 的 .vi 文件中。用户一般并不会意识到这种特殊情况，这种情况有个典型的应用，就是 [Express VI](measurement_express_vi)。几乎所有的 Express VI 都具有一个“设置对话框”，它们的参数在每个使用 Express VI 的地方，都可以被任意配置为不同的值。这些配置信息肯定是被保存在某处了，但发布的工程中却并没有保存 Express VI 的 .vi 文件。

实际上，Express VI 是一种特殊的“可配置节点”。当你配置完一个 Express VI 后，它通常会在后台动态生成一个标准的子 VI 代码。

这个生成的子 VI 包含了实际的执行逻辑。根据 Express VI 的类型不同，生成的代码有时是作为隐藏的子 VI 保存在内存中（随调用者 VI 一起保存），有时则是作为独立的 .vi 文件保存在项目的支持文件夹中。用户在使用时感觉不到它的存在，看似代码“融合”了，但本质上它还是基于标准 LabVIEW 节点的代码生成技术。
...
如果输入值是数组数据，它通过分叉的连线被同时输入到一个数组索引节点和一个加法节点，这构成了并行执行。

虽然 LabVIEW 的编译器非常智能，会进行“原地操作（In-Placeness）”分析，并尝试安排最优的执行策略以减少内存拷贝，但作为程序员，绝对不能依赖这种未明确定义的执行顺序。

如果逻辑上要求必须“先读取（索引）”再“修改（加法）”，你必须使用错误簇（Error Cluster）或顺序结构来强制规定执行顺序。否则，这就是一个典型的竞态条件（Race Condition），可能导致数据错误。

LabVIEW 确实有缓存重用（Buffer Reuse）机制，但它是在保证数据流逻辑正确的前提下进行的，而不是为了优化内存去改变数据流的并行规则。

注：为了显式地控制内存复用，LabVIEW 提供了“元素同址操作结构”（In Place Element Structure），这才是推荐的优化方式。
...
LabVIEW 为了解决这个问题，给 VI 提供了两种不同的可重入（Reentrant）属性：
- 预分配的克隆重入执行（Preallocated clone reentrant execution）： 如原文所述，LabVIEW 为该 VI 在每一个调用点都单独分配并保留一份数据区。优点是执行抖动小（无需等待），缺点是如果调用点很多，内存消耗会非常大。
- 共享的克隆重入执行（Shared clone reentrant execution）： LabVIEW 维护一个该 VI 的“克隆池”。当有调用请求时，从池中取出一个空闲的 VI 副本进行计算，用完后放回池中。如果同时有 100 个地方调用，但并发数只有 5 个，那么只需要 5 个数据区副本即可。这是平衡执行效率与内存占用的推荐方式。
...
当程序在不同平台下协同开发时，或者使用 Git/SVN 进行源代码管理时，强烈建议采用可执行代码与源代码分离的策略。

在 LabVIEW 项目开发中，建议在“项目属性”中设置默认将新创建的 VI 设为代码分离。这样可以彻底避免因为“重新编译”导致的文件二进制哈希值变化，从而解决版本控制中出现的大量“假冲突”问题。
...
这个 "Inline sub VI.vi" 被设置为内嵌子 VI。一个内嵌子 VI 必须是可重入的。内嵌子 VI 的代码在每个子 VI 被调用的地方都会有一个副本，数据空间就更是要每个调用地方都有自己的副本了。

由于内嵌子 VI 的代码在编译时会被直接“展开”并融合到调用它的 VI 中，运行时并不存在独立的子 VI 调用边界。因此，内嵌子 VI 并不支持常规的调试功能（如高亮执行、设置断点、查看探针）。

如果需要调试内嵌子 VI 的逻辑，通常需要先取消“内嵌”选项，调试完毕确认无误后，再重新勾选该选项以优化性能。
