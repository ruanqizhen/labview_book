...
64 颗棋子并非总是显示在屏幕上的。游戏一开始，屏幕上只有四颗棋子，以后每走一步多一颗棋子。LabVIEW 的每个控件都有一个 "可见（Visible）" 属性，用来输入控件是否需要在前面板上显示出来。棋盘的某个位置还没有放棋子时，可将该位置的棋子控件隐藏起来。这只需要一个非常简单的控件属性设置操作：
...
如果有一组控件需要同时出现或隐藏，还可以考虑利用选项卡控件。把这一组控件添加在选项卡控件的某个页面上，调整选项卡控件的显示页面，就控制了控件是否出现。如果把选项卡设置为透明，则可以更加灵活地输入控件的显示或隐藏。向导类型的界面是一种比较常见的界面类型，用户完成一个步骤的操作，点击界面上的 "下一步" 或者 "上一步" 按钮，就会出现一个包含了新内容的新界面。这类界面非常适合使用选项卡控件。选项卡控件的每一页对应向导中的每一个步骤。点击 "下一步" 或者 "上一步"，便可以切换到选项卡的另一页。
...
（注意： 使用 Get Control.vi 是为了方便演示，但这是 NI 的内部保留代码，随时可能在未来的版本中被删除或修改，导致程序无法运行。对于正是项目，建议避免使用这种内部 VI，而是应该使用前面板的 Controls[] 属性节点获取所有控件的引用数组，然后在 For 循环中通过读取“标签 -> 文本”属性来比对查找。）
...
这一属性也可以在程序运行时被动态改变，主要涉及两个容易混淆的属性：
- 前面板窗口 -> 透明度 (FP:Transparency)： 这个属性接收 0 到 100 的百分比数值。用于设置整个窗口的半透明（Alpha 通道）级别。设为 50 时，整个界面（包括按钮、背景）都会变成半透明，可以透视到桌面背景。
- 前面板窗口 -> 以透明方式运行 VI (FP:RunTransparently)： 这是一个布尔属性。当设为“真”时，它会完全隐藏操作系统的窗口边框和标准灰色背景，只显示界面上悬浮的控件图形。这其实是 LabVIEW 原生提供的一种实现“不规则形状界面”的方法，比调用 Windows API 更安全。

![](images/image662.png "VI 前面板在运行时变为半透明，可以透过它看到后面的程序框图")

### 调用 Windows API 实现不规则形状界面

有的程序界面需要的不是规则的矩形窗口，而是一个特殊形状。这种风格在媒体播放器软件中最常见。比如，常见的 mp3 播放软件，都具有 "换皮肤" 功能，以各种不规则形状来展现自己。

只需要调用几个简单的 Windows API 函数，LabVIEW 编写的程序也可以达到类似效果。
...
这种实现方式有两个兼容性问题：
- 64 位兼容性： 在现代 64 位 LabVIEW 和 Windows 环境中，绝对不要使用旧版的 GetWindowLong 和 SetWindowLong 函数。窗口句柄在 64 位系统中是 8 字节宽，旧函数会截断指针导致 LabVIEW 直接闪退。必须在“调用库函数节点 (CLN)”中调用 GetWindowLongPtr 和 SetWindowLongPtr，并将参数配置为“与指针大小相同的整数 (Pointer-sized Integer)”。
- 操作系统兼容性： 调用底层的 Windows API 意味着 VI 将彻底丧失跨平台运行能力（无法在 Linux RT 或 macOS 上运行）。如果不是必须追求异形界面的极致视觉效果，工业软件通常不建议采用这种强绑定操作系统的 Hack 做法。

更好的方法是使用“ 前面板窗口 -> 以透明方式运行 VI (FP:RunTransparently)” 设置。

### 原生不规则形状界面

实现一个优雅的跨平台异形透明窗口，核心在于结合 PNG 透明图片、VI 原生透明属性以及自定义鼠标拖拽逻辑。以下是具体的架构思路和实现步骤：

#### UI 素材准备与前面板设置

不规则形状窗口的视觉边界实际上是由一张带有 Alpha 通道（透明度）的图片决定的。所以，我们首先需要使用 Photoshop、GIMP 等图像编辑工具工具设计一个不规则形状界面的底图，保留边缘的透明度或阴影效果，导出为 `.png` 格式。将该图片直接拖入 LabVIEW 前面板，并右键选择“移至后面（Move to Back）”作为背景。

之后将需要的按钮、图表等控件放置在 PNG 背景图的有效区域内，并根据需要将控件背景设置为透明（使用工具选板的“T”画笔）。

进入 `VI 属性 (Ctrl+I) -> 窗口外观 (Window Appearance) -> 自定义 (Customize)`。取消勾选： 显示标题栏、显示菜单栏、显示水平/垂直滚动条、运行时显示工具栏。这是为了扒掉操作系统强加给窗口的“标准外壳”。

#### 开启原生透明

在程序框图的初始化阶段（如事件结构运行前的第一帧），调用 VI 的原生属性节点来剥离默认的灰色底板。

1. 在程序框图放置一个 “属性节点 (Property Node)”，默认关联为 `本 VI (This VI)`。
2. 左键点击属性，选择 “前面板窗口 (Front Panel Window) -> 以透明方式运行 VI (Run Transparently)”。
3. 将其更改为“写入”模式，并连线一个布尔常量 True (真)。

当该属性设为 True 时，LabVIEW 会自动将前面板原本默认的系统灰色背景（即未被控件或图片遮挡的部分）完全剔除，实现真正的“镂空”效果。此时，导入的 PNG 图片的轮廓就成了程序真正的物理轮廓。

#### 自定义窗口拖拽

剥离了操作系统的标题栏后，会面临一个致命问题：用户无法拖动这个窗口了。在 Windows API 时代，人们通常用 `SendMessage` 欺骗系统鼠标按在了标题栏上。在纯原生跨平台架构中，我们需要用事件结构 (Event Structure) 来自己计算鼠标偏移量。

我们需要一个并行循环（或集成在主事件结构中）来处理以下三个事件：

1. 窗格 -> 鼠标按下 (Mouse Down)：当用户在背景图上按下鼠标左键时，记录此时的 `鼠标坐标 (相对于屏幕)` 和当前的 `前面板窗口 -> 窗口边界 (Window Bounds)`。将一个“正在拖拽 (IsDragging)”的布尔标志位置为 True，存入移位寄存器。
2. 窗格 -> 鼠标移动 (Mouse Move)：如果“正在拖拽”为 True，读取当前新的 `鼠标坐标 (相对于屏幕)`。计算坐标差值：`新窗口位置 = 初始窗口位置 + (新鼠标坐标 - 初始鼠标坐标)`。将计算结果写入 `前面板窗口 -> 窗口边界` 属性节点，窗口就会跟着鼠标丝滑移动。
3. 窗格 -> 鼠标抬起 (Mouse Up)：将“正在拖拽”标志位置为 False，结束拖拽计算。

#### 淡入淡出效果

如果想让不规则形状窗口在启动时有一种高级的“淡入”效果，可以结合 `前面板窗口 -> 透明度 (Transparency)` 属性。初始时将 `Transparency` 设为 100（完全透明）。使用一个 For 循环，加上几毫秒的延时，将透明度从 100 递减写入到 0（完全不透明）。由于是原生属性，这个渐变动画在任何操作系统上都会非常平滑。
