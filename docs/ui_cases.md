...
（注意： 使用 Get Control.vi 是为了方便演示，但这是 NI 的内部保留代码，随时可能在未来的版本中被删除或修改，导致程序无法运行。对于正式项目，建议避免使用这种内部 VI，而是应该使用前面板的 Controls[] 属性节点获取所有控件的引用数组，然后在 For 循环中通过读取“标签 -> 文本”属性来比对查找。）
