...
有些程序的界面，一打开来就让人眼前一亮，这可能是因为它的界面设计得非常新颖、华丽。但漂亮的感官冲击并不代表着就是一个好的程序界面。评判一个界面优劣的最基本指标，首先是要看这个界面的交互功能是否完成。即用户是否可以通过界面为程序提供必要的信息、并接收到需要的信息。其次，是要看用户通过这个界面是否可以简单直观地输入或获取信息。最后才是界面的美观程度。

从这个角度说，一个设计良好的界面，通常应该是让用户感到十分自然，而并不引起过分注意的界面。在多数场合，引起用户对界面的注意反而往往是因为他们觉得刺眼别扭，或者是因为他们找不到所需的信息、或找不到输入信息的位置了。

使用 LabVIEW 开发一个项目，或编写一个软件，通常可以按照以下六个步骤进行：收集需求、设计、编码、测试、发布及维护。就设计阶段而言，一个项目所需的设计可能包括：用户界面设计，程序结构设计，接口设计，模块设计等等。编写 LabVIEW 程序，通常是从用户界面设计着手的。首先进行界面设计，可以使得界面设计不受程序实现的影响。若先设计程序结构，然后再设计界面，程序员难免会把简化编码作为主要目标，而把界面设计当成了配角。这样的界面通常不是用户使用起来最方便的界面。
...
一个能够让用户迅速接受并且方便操作的程序界面，它最为关键的一个特点应该是，这个界面保持高度的一致性。这里说的一致性包含了多个方面。
...
如果仅仅是为了在界面上显示一段无边框、无背景的提示文字，最简单的做法是：直接在前面板空白处双击输入文字，创建一个自由标签 (Free Label)。它是天然透明的。如果需要在代码中动态改变其内容，可以右键该标签选择“创建 -> 引用”，然后通过属性节点修改其文本。

如果需要使用输入框但希望看起来像纯文本，可以使用系统风格 (System) 选板下的“字符串指示器”，在设置为只读状态时，它在现代系统上常常表现得非常融洽。

在一些高级 UI 定制中（例如制作自定义形状的按钮），我们需要控件完全透明时，可以调出工具选板的颜色工具，使用“T”透明色去涂抹经典风格 (Classic) 控件的边框和背景：
...
银色风格引入于 2011 年，带有强烈的金属反光质感。虽然当年很流行，但在如今推崇“扁平化设计（Flat Design）”的趋势下，银色控件显得过于臃肿和过时，不建议在现代软件中大量使用。

NXG 风格是 LabVIEW 近年引入的扁平化风格控件，去除了冗余的 3D 阴影，线条干净利落。在开发现代、专业的工业软件界面时，强烈建议首选 NXG 风格控件，或者下载第三方的 Flat UI 控件包。
...
对于应用程序界面，大家最习惯的就是 Windows 默认的界面风格了：使用窗口，窗口最上方是标题栏，下面是菜单，然后是工具条，再下面是主体内容，窗口最下方是状态栏，右侧以及最下面是滚动条。如果一定非要标新立异，把标题栏和滚动条的位置互换一下，那么用户肯定会觉得这个界面非常别扭。比如，微软推出的 Office 2007，界面风格有了重大改变，新的界面漂亮了不少。但是它的很多用户常用菜单、按钮都变更了位置，用户又要重新熟悉新的界面布局。所以在 Office 2007 发布后，很多用户宁可继续使用自己熟悉的旧版本 Office 软件。
...
下图是 LabVIEW 自带范例中的一个 VI。尽管它是早期设计的，用的还是老式的控件，不如现在的程序界面那样柔和美观。但是，由于它在设计上与真实示波器保持一致：控件的外观、布局和操作方式等，都与真实示波器上的按键旋钮等相类似。所以，曾使用过示波器的用户，基本上不需要再查看这个 VI 的帮助文档，直接就会使用这个 VI 提供的类似示波器的功能了。
...
这个规范可以由公司内部定义，也可以遵循现有的行业规范。凡是开发 Windows 系统风格的程序，可以遵循微软官方的设计规范（可通过搜索 Windows App Design Guidelines 或 Fluent Design System 获取）。

对于纯正的 LabVIEW 程序，应当遵循 NI 官方的界面开发规范。建议在 NI 官网或 LabVIEW 帮助文档中搜索 "LabVIEW Style Guide"。这份指南详细规定了面板背景色的 RGB 值、默认字体的选择（如 Segoe UI 或 Tahoma）、控件对齐的像素间距等极具操作性的细节。

## 界面元素的关联

![](images/image575.jpeg "原来的菜单")    
...
设计用户界面时，要照顾到新用户。为了方便新用户了解界面的使用方法，需要给他们提供足够的辅助或帮助信息。LabVIEW 主要是通过以下几种手段给用户提供帮助信息的：用户手册；即时帮助窗口；提示条；利用控件的标题、选项文字；直接把帮助文字写在界面上。
...
在 LabVIEW 程序中，通常可以把相关的链接写在即时帮助窗口中：
...
除了枚举和下拉列表型控件，单选按钮也可以起到同样的效果。单选按钮有一个优势，就是可以直接在界面上显示出所有可供选择的值，并且可以附带对每个选项的详细解释。如果界面的空间比较宽松，可以考虑使用单选按钮控件。比如下图中的界面，这是 VI 属性中的一页，用于设置 VI 密码。关于 VI 保护的三种选择被一目了然地显示在界面上。
...
在控件的右键菜单中选择 "创建 -\> 属性节点 -\> 禁用"，在属性节点的输入端右键点击，选择“创建 -> 常量”。LabVIEW 会自动为你生成一个包含 “启用 (Enabled)”、“禁用 (Disabled)”、“禁用并变灰 (Disabled and Grayed Out)” 三个选项的枚举常量。选择“禁用并变灰”是 UI 交互中最标准的防错视觉反馈。
