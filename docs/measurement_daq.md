...
电脑上可能安装有同一个类型的多种型号仪器的专用驱动程序。类驱动程序应当调用哪一个专用驱动程序，可以在 IVI 配置文件中指定。安装 LabVIEW 时，还会同时安装一个叫做 "Measurement & Automation Explorer"（简称 MAX）的软件。这是 NI 公司专用于配置计算机硬件设备的应用软件，通过它可以比较直观地修改 IVI 配置文件中的设置。
...
但有些程序对定时的精度要求非常高。比如，在利用数据采集设备对信号进行采集时，其精度要求可能是前面提到的程序的数千倍以上。高速数据采集时，时间误差要求甚至可低至 10^-9^ 秒（纳秒）级别。
...
更简单的定时方法是直接使用 "等待"、"等待下一个整数倍毫秒" 或 "时间延迟" 函数，这三个函数的调用方法相似。它们可以分辨的最小等待或延时时间也是相同的：
1 毫秒。它们唯一的差别在于精度。"等待" 与 "时间延迟" 的精度是相同的，它们每执行一次的误差可达数个毫秒。"等待下一个整数倍毫秒" 的精度要高一些。
...
只有在要求循环周期保持稳定，且不能有长期累积误差时，才需要深刻理解它们的底层差异：
- “等待” (Wait ms)：相对定时。 它的计时起点是该节点开始执行的瞬间。假设循环里有其他耗时代码（耗时 $t$），则单次循环的总周期为：$t + \text{设定等待时间} + \text{系统调度误差}$。随着循环次数增加，误差会被不断累加。
- “等待下一个整数倍毫秒” (Wait Until Next ms Multiple)：绝对同步定时。 它不关心循环里的其他代码耗时多久，它紧紧盯着操作系统的内部毫秒计数器 (Tick Count)。如果设定参数为 100，它只会在系统计数器达到 100, 200, 300, 400... 的绝对时刻被唤醒。因为它的唤醒时刻是预先锚定在系统时钟轴上的。即使某一次循环里的代码偶尔卡顿多耗了 5ms，它下一次唤醒的绝对时刻依然不变，自动把这 5ms 的延误“吸收”掉了，从而保证了长期的平均频率绝对准确。
...
此外，即使是在 PC 机上运行的程序，遇到需要精确定时、需要设定多个不同级别的多个定时、或者需要动态改变定时功能这三种情况，也应该考虑使用定时循环。定时循环位于 "编程 -\> 结构 -\> 定时结构" 函数子选板中。该子选板内包括了 "定时循环"、"定时顺序" 等节点。

必须明确一点，在非实时操作系统（如标准的 Windows）下，无论使用“等待”函数、“等待下一个整数倍毫秒”函数，还是看起来更高级的“定时循环 (Timed Loop)”，它们的时间精度极限是相同的，通常误差都在几毫秒级别。这是由 Windows 自身的线程调度机制决定的。

那么，“定时循环”的真正优势是什么？
- LabVIEW RT 系统的确定性： 如果程序运行在 LabVIEW Real-Time (RT) 嵌入式硬件上，定时循环能够提供微秒级的绝对精确定时，这是普通循环做不到的。
- Windows 下的调度控制： 在 Windows 下，定时循环允许指定线程优先级（优先抢占 CPU）和多核亲和性（强制绑定在某个 CPU 核心上运行，减少缓存未命中）。这能减少时间抖动 (Jitter)，但不能突破 Windows 毫秒级的理论精度。此外，它还能方便地管理多个不同周期的并行任务的相位同步。
...
实际上，专业的数据采集设备（如 NI 的 DAQ 采集卡或高档声卡）内部都配有独立的、高精度的硬件晶振（时钟源）。

在数据采集中，绝对不能在软件中写一个 While 循环加上“等待 1ms”函数去一笔一笔读数据。正确的做法是：在初始化阶段，通过驱动程序（如 DAQmx 配置节点）告诉采集卡你需要的 采样率 (Sample Rate，单位：Hz/Samples per second)。

启动后，采集卡会完全根据自己内部的硬件晶振，以极高且绝对精确的频率（比如 100 kHz）将数据源源不断地塞进它自带的硬件缓冲区（FIFO）。你的 LabVIEW 软件循环只需要以一个较慢的频率（比如每秒 10 次），每次从缓冲区中一次性“打包”读出 10000 个数据即可。这就是高效且无误差的硬件定时缓冲采集 (Hardware-Timed Buffered Acquisition) 模式。
