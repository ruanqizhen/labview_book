...
在 LabVIEW 中，类的属性（数据）只有一种访问权限，私有，数据只能被同一个类中的 VI 读写。LabVIEW 中，类的方法（VI）倒是支持了以上所有四种访问权限的设置，它在类的设置对话框中：
...
比如，我们上一节中使用的家具店的例子，家具类中数据访问方法只是给子孙类中的方法使用的，所以应当设置为 “保护”（Protected）权限。
...
需要格外注意的是，LabVIEW 中的 “基于动态分配模板” 和 “基于静态分配模板” 与讨论其它编程语言时候所说的动态静态是完全不同的概念。按照大多数编程语言对静态动态的定义，我们之前演示过的 LabVIEW 类中的属性和方法大多是属于动态的：类中 VI 都有一个 “类” 输入控件，并且是必须的。也就是说，不输入一个类的实例进来，这些 VI 是不能被调用的。LabVIEW 的类中也可以添加 “静态方法”，只要添加一个普通的 VI，没有类输入控件的就可以了。比如下图中的 VI，它虽然被放置在一个类中，但并没有类的输入，因此可以在任何地方被直接调用。
...
接口虽然解决了一个对象同时支持多类功能的问题，可以让这个对象被多个种处理不同类型数据的程序接受。但是它并没有解决有效降低重复代码的问题，因为很多方法明明已经在其它类中定义了，却不能继承。不同的编程语言采取了一些不同的措施来更有效的重复利用代码。比如在 PHP 语言中引入了一种叫做 Traits（特性）的东西。一个特性是一个代码块，它的封装方式与类是有些相似的。在其它类中，可以使用一个定义好的特性。比如我们定义一个 TableTraits，封装一些桌子会用的方法，比如 put_tablecloth 方法。我们还有一个叫做 DeskClass 的类，使用了 TableTraits 这个特性，这样 DeskClass 就拥有了 TableTraits 封装的所有方法。一个特性可以被用在多个类中，一个类也可以使用多个特性。比如，另一个叫做 DiningTableClass 的类已使用了 TableTraits 这个特性，DiningTableClass 就也拥有了 put_tablecloth 方法。
...
Java 在解决这个问题的时候，采取了另一种策略，他允许在接口中给函数添加默认实现。如果使用了接口的类没有重写这个方法，那么，它的对象就会使用接口中实现的这个方法。接口中的方法有了实现，又有多继承，如果不加以限制就和类的多继承没区别了，就又会引起多继承的所有问题。所以 Java 对于接口中方法的默认实现有一个限制：当某个类实现了多个不同接口，并且这些接口中有同名方法且都提供了默认实现时，该类必须重写该方法。这样，无论哪个程序调用该类的对象，都能明确知道调用的是该类中重写的方法，而不会调用接口中的默认实现。这样就避免了，在多继承中搞不清楚是调用关系的问题。
...
假如一个主 VI A.vi 被装入内存，它的子 VI B.vi 也会被装入内存，和 B 同属一个类的 VI C.vi 也要被装入内存， C 中有个子 VI D.vi， D.vi 属于类 E.lvclass， E.lvclass 的父类是 F.lvclass，F.lvclass 中有个方法 VI G.vi。尽管 G.vi 的功能和程序 A.vi 根本不挨边，但也会被装入内存。

此外，还有一个导致灾难性加载时间的元凶，是糟糕的架构导致的交叉依赖 (Circular Dependency)。比如：A 类的方法中调用了 B 类；而 B 类的方法中为了做某种类型转换，又放置了一个 A 类的常量或调用了 A 类的方法。这种你中有我、我中有你的网状耦合，会迫使 LabVIEW 在启动时进行海量的交叉编译和内存分配验证。

因此，设计类时不仅要注意拆分，更要严格遵守依赖单向流动（如底层驱动不应该知道顶层 UI 类的存在）。这也是我们在[类的设计方法和原则](oop_solid)一节要重点探讨的。
...
2.	类中的 VI 必须是高内聚的，即类中的方法共同完成某一基本功能，不可再分割。应用程序一旦用到这个类中的某个 VI，就意味着程序将会使用到类中几乎全部的 VI。如果某个应用程序可能只需要使用这个类中的某几个 VI，那就就当把这个类拆分成多个小类。
3.	继承关系应当尽量简单，没有必要的时候尽量不使用继承。
4.	打算使用类的多态特性时要注意，多态使得应用程序在运行时，会根据对象的类型选择对应的处理方法。但有些选择应当是程序编译时就做出的，它们不适合套用在多态特性上。
...
以上仅仅列举了一些在设计“类”的时候，需要考虑的最简单的规则，在[类的设计方法和原则](oop_solid)一节中，我们将会对这个问题进行更深入的探讨。
...
虽然像上图那样在程序框图硬放一个子类常量能解决问题，但这在软件工程中是极度反模式（Anti-Pattern）的。面向对象的核心就是为了解耦，主程序不应该静态依赖所有的子类。正确的做法是利用工厂模式（Factory Pattern）和动态加载技术：
1. XML 数据或配置文件中通常会记录该对象的具体类名或类路径。
2. 在执行反序列化之前，使用 LabVIEW 提供的 “获取 LV 类默认值（按名称/路径）(Get LV Class Default Value)” 函数。
3. 这个函数会在运行时动态地将子类加载进内存，并输出一个空的默认对象实例。
4. 将这个动态加载的实例连线到 XML 反序列化节点的“类型（Type）”输入端，即可完美、解耦地完成数据恢复。
