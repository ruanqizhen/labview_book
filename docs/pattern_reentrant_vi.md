...
但是我们还可以换一种方法来考虑如何计算阶乘：我们不是直接从原始的数据开始计算，而是用归纳的方法，一步一步的来简化问题，比如6的阶乘，它可以通过先计算5的阶乘，再把结果乘以6。用公式来描述就是 `0! = 1, n! = n * (n-1)!`，或者写成函数的形式为：`F(0) = 1, F(n) = n * F(n-1)`。数学归纳法中一定要有一个基本情况，对于阶乘的问题，基本情况是0，0的阶乘等于1，这是人为定义出来的。有的读者可能已经注意到了，用归纳法表示的阶乘与直接计算阶乘不是完全一模一样，归纳法把阶乘计算推广到了所有非负整数，而直接计算，只能计算正整数的阶乘。算是归纳法的一个小优势吧。
...
* 上面的分支去计算19的斐波纳契数，在起内部肯定需要计算18的斐波纳契数；
* 下面的分支去计算18的斐波纳契数，可是18的斐波纳契数已经在上面那个分支里计算过了啊，现在又要重复计算一遍！

这样的算法，输入值每增加1，程序的计算量就要翻倍。这是编写递归程序容易出现的一个问题，同样的计算，在不同的地方被运行了很多次。使用循环通常可以解决这个问题：求n的斐波纳契数，可以从0开始计算，在计算出1的斐波纳契数，再计算2的斐波纳契数……循环迭代n次，得到n的斐波纳契数。

但很多时候，使用循环编写的代码，不如递归简洁易读。使用递归同样也有办法解决上面重复计算的问题。读者可以思考一下，有什么简单的方法能够即使用递归，又避免重复计算？
