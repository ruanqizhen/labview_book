...
VI Scripting（VI 脚本）是 VI 服务器提供的一套属性和方法，通过调用这些属性和方法，就可以编写程序去调整修改某些 VI 的前面板，程序框图，也可以修改由 VI 生成的应用程序，甚至是 LabVIEW 中的一些设置。VI
...
### VI 服务器与 VI 脚本 (VI Scripting) 的区别

在 LabVIEW 中，通过编程控制对象的功能统称为 VI 服务器 (VI Server)。我们在本章中讨论的“在运行时改变控件位置、颜色”、“动态装载并运行子 VI”等，都属于 标准的 VI Server 功能。初学者容易将它与 VI 脚本 (VI Scripting) 混淆。

- VI Server： 主要操作前面板（界面）和 VI 的执行状态。这些功能在开发环境和最终生成的 EXE 应用程序中通常都是可用的。
- VI Scripting： 是 VI Server 的一个高级子集，专门用于通过代码操作程序框图（Block Diagram）（例如：自动生成一个 While 循环，或者自动将两个节点连线）。VI Scripting 主要用于编写开发辅助工具，且极度依赖编辑环境，在生成的 EXE 应用程序中通常是无效的（因为 EXE 默认移除了程序框图）。

与 VI Server 相关的节点都在函数选板 “编程 -> 应用程序控制” 中。要启用底层的 VI Scripting 功能，需要在 LabVIEW 的“选项” -> “VI 服务器”页面中专门勾选“显示 VI 脚本函数、属性和方法”。
...
虽然使用属性节点（尤其是通过引用调用）非常灵活，但在架构设计时必须极其谨慎。LabVIEW 中绝大多数与前面板控件相关的属性节点，其底层执行被强制绑定在 用户界面线程（UI Thread） 上。

如果在一个执行密集型运算或高速数据采集的循环中，频繁调用属性节点（例如每次循环都去读取控件的“值”属性，或改变“颜色”），LabVIEW 被迫在“执行线程”和“UI 线程”之间来回切换。这种线程切换的开销极大，会导致程序运行速度断崖式下降。

最佳实践： 绝不要在高速循环中频繁调用界面属性节点。如果只为了传递数据，请优先使用数据连线或局部变量；如果必须更新界面状态，请考虑降低更新频率（例如使用定时器，每 100ms 更新一次）。
...
尽管通过 Panel -> Controls[] 属性获取所有控件数组，然后根据标签（Label）逐个比对查找是一种可行的方法，但这种做法强烈不推荐用于正式项目。因为：
- 性能极低： 遍历操作非常耗时。
- 脆弱性： 这种做法依赖于控件的字符串标签。一旦有人在界面上修改了控件的文字标签，后台代码就会因为找不到对应的控件而悄无声息地失效。

如果需要动态操作一组特定的控件，最稳健的做法是在初始化阶段，将这些控件的直接引用（右键控件 -> 创建 -> 引用）手动打包成一个数组或簇，甚至存入映射表（Map）中，并通过移位寄存器在循环中传递。这样既保证了绝对的类型安全，又拥有极高的执行效率，且不怕界面标签的修改。
...
一个对象的引用可以通过使用 `编程 -> 应用程序控制 -> 转换为更具体的类 (To More Specific Class)` 或 `转换为更通用的类 (To More Generic Class)`，在继承树中进行转换。
- 向上转型 (Upcasting)： 例如把“波形图”引用转换为“控件”引用。通常不需要显式调用节点，LabVIEW 在连线时会自动进行隐式转换。
- 向下转型 (Downcasting)： 这是实际编程中最常用的。例如，通过 Controls[] 属性获取了一个数组，里面全是通用的“控件”引用。我们知道其中第二个元素实际上是一个“布尔”控件，并且想改变它的颜色（这是布尔控件特有的属性）。此时，我们必须使用“转换为更具体的类”节点，将其强制转换为“布尔”类型，然后才能连接到包含“颜色”属性的节点上。

注意： 向下转型具有运行时的不确定性。如果你试图将一个实际是“字符串”的通用控件引用强制转换为“布尔”类，该节点将在运行时报错。因此，务必连接错误簇以处理潜在的类型不匹配问题。


比如下面的程序，使用“转换为通用的类”把几个不同控件的引用都表示为“控件”类，然后通过“控件”类的属性从这些控件读取了它们的标签信息：
...
属性节点也可以用于查看或控制局域网内其他计算机上的 LabVIEW 应用程序。这需要使用“打开应用程序引用”函数，并输入目标机器的 IP 地址。

跨机器调用 VI Server 功能受到严格的安全限制。必须在目标机器的 LabVIEW “选项 -> VI 服务器”中，正确配置“机器访问 (Machine Access)”白名单，并确保操作系统的防火墙放行了指定的端口（默认通常是 3363）。在复杂的网络环境中，这种做法配置繁琐，现代架构往往更倾向于使用 Network Streams 或 Web Services 来代替底层的远程 VI Server 调用。
