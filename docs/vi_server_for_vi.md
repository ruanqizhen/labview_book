...
在这一章的某些示例图中，为了保持代码结构简单，省略了一个极其重要的步骤：关闭引用（Close Reference）。在实际编写 VI Scripting 程序时，通过“创建 VI 对象”或任何属性节点获取的每一个对象引用（无论是节点、连线还是接线端），在使用完毕后都必须显式调用“关闭引用”函数。如果不关闭这些引用，LabVIEW 会产生严重的内存泄漏。在处理批量生成或修改任务时，这会迅速耗尽系统内存导致 LabVIEW 崩溃。

需要通过编写程序从零创建一个新 VI 的用例其实并不多，除非是需要生成大量高度模板化的硬件驱动程序 VI。VI Scripting 最主要的用途是开发编辑器辅助工具，例如：编写“快捷放置插件 (Quick Drop Plugins)”来一键自动连线、编写自定义的右键菜单插件，或者开发代码审查和自动重构工具。此外，运行程序来编写 VI 的程序框图也是 [XControl](ui_xcontrol) 和 [XNode](oop_xnode) 的实现方法。

## 批量修改 VI

在实际应用中，比较常用的是需要对已有的 VI 进行某种批量修改。比如，已经编写好了一个由几十个 VI 组成的项目。随后，又有新的需求，要求修改所有 VI 的某属性，或者统一修改所有 VI 中某些不规范的控件标签名，甚至修改所有 VI 的前面板背景色等。这些修改在每个 VI 中都是相似的，这种情况就非常适合通过 VI Scripting 编程来统一处理。

以一个简单的任务为例。当我们需要把自己所写的一批 VI 发布给外部用户时，可能需要给这些 VI 加上密码，以保护知识产权。

要完成这样一个任务，首先要得到所有需要被修改的 VI 的路径。在这个例子中，我们采用最简单的策略，列出一个文件夹下的所有 VI 文件名。然后一一打开每个 VI 的引用，通过属性和方法对 VI 进行修改。在这个例子中，我们使用 VI 的“锁定状态 -> 设置 (Lock State -> Set)”方法给 VI 添加密码。

修改了 VI 之后，一定要执行保存操作。如果不保存，一旦 VI 引用被关闭，内存中的所有修改都会随着 VI 卸载而丢失。保存 VI 使用的是 VI 的“保存 -> 仪器 (Save -> Instrument)”方法。（注：这里的“仪器”是早期英文版 Virtual Instrument 缩写带来的历史直译，它实际上指代的就是内存中的这个 VI 实例本身）。

最后，千万不要忘记在每次循环结束前关闭当前 VI 的引用。整个任务的逻辑流程如下图所示：
