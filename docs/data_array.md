...
簇控件中元素控件的摆放位置，与簇数据中元素数据的逻辑顺序不一定是一致的。在程序框图中，处理簇数据时，只有元素数据的顺序是有意义的，与控件的摆放位置就不再相关了。当簇中元素比较多的时候，应避免元素随意摆放，可以让簇自动排布内部的元素。在簇的快捷菜单中选择“自动选择大小 -\> 垂直排列”，这样，簇中元素个数变动时，不必再手动调整簇的外观。当选择自动排列时，簇中控件的位置顺序，就是簇中元素数据的顺序，这样保持一致，可减少顺序混淆的可能性。用鼠标把一个控件挪动到另一控件上面，是不会改变元素的顺序的，如果需要调整簇中元素的顺序，需要在簇的右键菜单中选择“重新排序簇中控件”。

在簇的右键菜单中选择“重新排序簇中控件”后，簇内每个元素上会出现两个数字：白底黑字代表当前的逻辑顺序，黑底白字代表修改后的新顺序。你需要按照你期望的顺序，依次点击每个控件。点击后，该控件的“新顺序”数字会随之更新。设置完成后，点击工具栏上的“确认”按钮（或簇边框上的确认勾号）保存更改：
...
数组元素的数据类型虽然是唯一的，但数据的个数可以是非常多的（理论上，在 32 位 LabVIEW 中，每个维度可以最多包含 $2^{31}－1$ 个元素。在 64 位 LabVIEW 中，虽然理论上索引可达 $2^{63}-1$，但实际大小主要受限于计算机的物理内存（RAM）。对于巨大的数组，内存往往是先于索引范围触顶的瓶颈）。数组控件左上角有一个额外的数字显示，它表示的是当前显示的数组元素的索引，也就是第几个元素。数组元素索引是从 0 开始计算的，第一个元素的索引是 0；第二个元素的索引是 1，以此类推。
...
索引多维数组的时候，可以索引某个元素，也可以索引某个维度。比如需要索引二维数组某个元素时，就要求“索引”参数输入两个位置信息。LabVIEW 遵循 “先外层后内层” 或 “先每一个行 (Row) 后每一个列 (Col)” 的原则。当“索引数组”函数有两个索引输入端时，上面的接线端对应行索引，下面的接线端对应列索引：
...
如果同时有多个索引输入隧道，与他们相连的数组长度不一致，则循环迭代次数为这几个数组中，长度最短的那个数组的长度。如果同时也为数组提供循环次数值，即输入一个 N 值，且数组输入隧道开启了索引功能，那么，循环的次数为这几个数组的长度和 N 值中最小的一个。如果输入数组的隧道禁用了索引，则该数组长度不影响循环次数，循环次数完全由 N 决定。
...
如果必须在 While 循环中构建大数组，但又无法确定确切大小，通常建议先在循环外初始化一个足够大的数组（预分配内存），利用移位寄存器传入循环，在循环内使用“替换数组子集”来填充数据，最后在循环外截取实际使用的长度。这样可以避免循环中频繁的内存重新分配。
...
从这一运行结果可以看出，这两个子 VI 实例共享了同一份状态数据。这是因为该子 VI 默认是非重入（Non-Reentrant） 的。在 LabVIEW 中，非重入 VI 在内存中只有一份数据副本。无论在程序中调用多少次，它们都指向同一个内存地址，因此其中的反馈节点（或未初始化的移位寄存器）就像一个全局变量一样被所有调用者共享：
