...
异步 I/O 并不是多线程，它是编程语言自己管理的运行调度系统，就类似与 LabVIEW 的执行系统。其实，绝大多数时候，使用多线程都是为了解决某些慢速的过程，比如文件读写、网络连接等操作，阻塞整个程序的问题。对于异步 I/O 的支持让这个问题的解决变得异常简单了，只要把慢速的函数设置为“异步”，编程语言的执行系统，就会在这个函数的等待期间去运行其它函数代码，避免整个程序被阻塞。也不需要使用者特别注意数据同步、安全之类的问题。
