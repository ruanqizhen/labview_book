...
为了解决这个问题，1990 年开始，计算机业界开始研发一种新的编码标准，它可以覆盖全世界所有的字符，也就是说，任何一个字符都有自己独占的编码，在任何系统下都会保持这个编码，这样就不会出现换个系统就乱码的问题了。这就是 unicode 编码，也叫万国码、单一码。unicode 规定了统一的字符集，但是这套字符集也依然还存在多种不同的编码格式。Windows 采用了 UTF-16LE 格式的 unicode 编码（UTF全称为 Unicode Transformation Format），使用 16 位的(双字节)数据表示一个字符。但是当前最流行的 unicode 编码格式却是 UTF-8，这是一种变长的编码方式，根据字符的常用程度，使用不同长度的编码来表示这个字符，编码长度有可能是 1 到 6 个字节。目前大多数的 unicode 文档采用的都是 UTF-8 编码。

经过几十年的推广和发展，现在已经很少有不支持 unicode 的主流软件了。但是，非常遗憾的是 LabVIEW 始终没有支持 unicode。在整个 2010 年代，NI 公司的重点放在了开发支持 unicode 的 LabVIEW NXG。计划用它彻底取代 LabVIEW，所以没有投入足够的资源去改进 LabVIEW。但后来，NI 公司又放弃了 LabVIEW NXG。目前，不支持 unicode 的 LabVIEW 仍然是使用者的唯一选择。

虽然 Windows 很早就开始支持 unicode 了，但是为了兼容那些还没有支持 unicode 的软件，Windows 系统保留了一个默认字符集的设置。对于非 unicode 的软件，Windows 会使用默认的字符集来解释那些字符编码。国内使用的 Windows 操作系统，无论是中文版还是英文版，几乎都会把默认的字符集设置为中文字符集。所以，一个软件是否支持 unicode，对于绝大多数用户来说，都是没有太大差别。笔者之前也没意识到 LabVIEW 不支持 unicode 会有什么问题。

直到最近，笔者使用了不同的操作系统，才开始注意到这个问题的严重性。笔者家里有两台电脑，一台操作系统安装了 Windows；另一台操作系统是中文 Deepin Linux。两台电脑上都安装了社区版的 LabVIEW 2021。笔者发现，在 Windows 系统下编写的 VI，如果包含中文常量或注释，拿到 Linux 系统下打开，看到的就全部是乱码；反之亦然。更严重的是，如果 VI 的名字中也包含了中文字符，那么，把项目拿到另一个系统下，很可能就无法打开了。

造成这些问题的根源在于 LabVIEW 没有直接支持 Unicode，它总是采用操作系统的默认字符编码对字符进行编解码：在中文 Windows 系统下使用 GB18030 中文编码；在 Linux 系统下使用 UTF-8 编码。

因为 Linux 系统普遍采用 Unicode，VI 在不同版本的 Linux 系统上的行为和显示内容是一致的。但 Windows 系统本身虽然采用 Unicode 编码，为应用程序提供的却是可由用户设置的默认编码。假设在一个项目中有两个 VI，分别是"界面.vi"和"任务1.vi"，其中"任务1.vi"是"界面.vi"的子 VI。在操作系统层面，两个文件"界面.vi"和"任务1.vi"的文件名使用 Unicode 编码保存。但在"界面.vi"内部，记录其调用的"任务1.vi"这一字符串时，使用的却是非 Unicode 编码（例如在中文 Windows 系统下使用 GB18030）保存的。因此，LabVIEW 内部记录子 VI 名称的二进制数据，与操作系统中记录该文件名的二进制数据并不一致。每次 LabVIEW 需要操作系统加载子 VI 文件时，还需进行编码转换。若保存和读取均在中文 Windows 环境下进行，编码始终一致，不会出现问题，VI 名称总能被正确转换。

但若将此前在中文 Windows 下保存的项目拿到非中文 Windows（默认编码不再是 GB18030）下打开，或拿到 Linux 上打开，VI 文件名在操作系统层面显示仍正确（因为操作系统采用 Unicode），但在 LabVIEW 内部打开时就会出错——LabVIEW 会尝试用新系统的默认编码来解释原本以 GB18030 格式保存的子 VI 名称，自然无法匹配正确的 VI 文件名，也就无法找到对应子 VI。同理，中文 Windows 下保存的、包含中文文件名的 VI，拿到 Linux 上打开也会出错；反之亦然。

总之，由于 LabVIEW 在 Windows 下未使用 Unicode，导致中文（或其它非英文文字）在跨平台场景下显示行为不一致，切换系统就可能出现乱码甚至 VI 无法加载。目前笔者也未找到完善的解决方案，只能尽量在 LabVIEW 中使用英文，避免使用中文及特殊字符。

如果有一天 LabVIEW 开始支持 unicode 了，那么这很可能意味着用户将无法再使用之前版本 LabVIEW 编写的程序：如果原有的 VI 中包含有其它语言文字，那么将会无法在 unicode 版本的 LabVIEW 中打开，或是打开后出现乱码。这将会是一个非常严重的兼容问题。这可能也是 NI 公司迟迟无法下决心支持 unicode 的原因之一。
...
LabVIEW 一直以其可视化界面设计著称，通过拖拽控件即可快速生成交互式程序界面。然而，如今的 AI 工具往往只需一句自然语言指令。比如，给 AI 工具下面的提示：“生成一个包含一个 3D 图表，两个数据输入控件，分别代表 X 和 Y 的值，然后再图标内用不同颜色显示出 Z=X*Y 的值”，它可在几秒钟内产出一个高质量的 Python 程序，然后直接运行就能够实现你希望看到的效果。前端界面布局与交互逻辑本就容易用自然语言描述，因此 AI 在这一领域的效率甚至超越人类工程师。例证之一是在 Web 开发领域，AI 对前端工程师的冲击速度远超后端工程师。同样，LabVIEW 在快速原型设计和 UI 开发上的传统优势，正被 AI 以更通用、更灵活的方式取代。
...
最近，我用 AI 编写了一个网页版的演示程序，生成了一个类似 LabVIEW 的图形编程环境。这不是一个真正的实用程序，只是想测试一下目前 AI 编程的能力，所以仅仅实现了几个最基本的功能：[https://webg.qizhen.xyz/](https://webg.qizhen.xyz/) 。 在只能利用业余时间的情况下，编写这样一个程序也只用几天而已。我的大多数编程技能都已经被 AI 淘汰了。

### 未来展望

与其他一些主流编程语言相比，LabVIEW 在其擅长领域中，往往开发效率更高，但运行效率更低。但 AI 编程正在重塑软件开发的生态，主流文本语言在 AI 的加持下，开发效率大幅提升。LabVIEW 面临着一个严峻的问题，当主流文本语言具备相同甚至更高的开发效率时，用户有什么理由去选择运行效率相对较低的 LabVIEW？

未来，LabVIEW 的应用空间很可能将被压缩到那些它最具不可替代性的领域，例如实时控制与工业自动化等。在这些对确定性和可靠性要求极高的场景中，LabVIEW 与其专用硬件平台（如 CompactRIO、PXI）在实时操作系统（RTOS）上的表现，仍具有无可比拟的优势。

当然，在其他领域，LabVIEW 也并非没有机会。它或许可以尝试融合生成式图形模型（Generative Graph Models）、多模态模型（Multimodal Large Language Model）等新方向，让 AI 直接生成可执行的图形化代码。如果能够实现这种跨越，LabVIEW 或许能在 AI 时代重新定义自身的价值。
