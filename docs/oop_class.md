...
默认状态下，LabVIEW 中每个新创建的类，其在程序框图上的数据线都是相同的深棕色粗线。为了在复杂的程序框图上清晰地分辨出一根数据线传递的是哪一个具体的类，我们可以为其设置自定义的外观。

右键点击项目树中的类，选择“属性 -> 连线外观”。

在修改连线颜色时，为了照顾色弱/色盲开发者，或确保在黑白截图/打印时依然能区分数据流，强烈建议不要仅仅修改颜色，应当同时在连线上添加文字标识（如类名的缩写）或更改线条样式（如虚线交替）。例如，为 Table 类的连线加上 "Tab" 字样，为 Chair 类的连线加上 "Chr" 字样。

为了在演示程序中更清楚的分辨一根数据线传递的是哪一个类的数据，演示程序为每个类都设置了不同的数据线颜色和样式：
...
* 最右边一部分，最右边一部分，把两把椅子和一张桌子放置在了同一个“创建数组”节点中。此时，发生了一个非常重要的面向对象动作：自动向上转型 (Upcasting)。因为“桌子”和“椅子”拥有共同的父类“家具”，LabVIEW 编译器会自动将这条数据线以及生成的数组类型提升（泛化）为它们最近的共同祖先——家具类 (Furniture Class) 数组。如果不具备这种继承关系（比如你试图把一把椅子和一个普通的数值放进同一个数组），连线将会断开并报错。
