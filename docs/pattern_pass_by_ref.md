...
一般来说，每调用一次“Obtain Queue”就要相应的调用一次“Release Queue”，队列的获取和释放是有计数的，要一一对应。但是，如果“Release Queue”函数的“force destroy”（强制销毁）参数被设置为“真”的话，这个“Release Queue”函数就会立刻把队列释放掉，其它线程就无法再访问这个队列了。
