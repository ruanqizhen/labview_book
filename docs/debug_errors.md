...
这个 VI 完成的功能是一个简单的乘法运算：300×300，不假思索就可以知道答案是 90000。但程序运行后，给出的结果却是 24464。乘法节点是不会错的，错误是由于程序中使用的数据类型是 I16。

$300 \times 300 = 90000$。90000 的十六进制表示为 `0x15F90`。由于 I16 数据类型仅占 16 位（2 字节），高位的 `0x1` 被截断丢弃，剩下的低 16 位是 `0x5F90`。`0x5F90` 对应的十进制正数正是 24464。

为了避免此类错误，在程序中使用短数据类型时，一定要确认程序中的数据绝不会超出该类型可以表示的范围。比如，在进行乘法运算之前，显式地使用“转换”函数（如“转换为 I32”），将 I16 数据强制转换为 I32。这样，乘法节点会自动输出 I32 类型的结果，从而容纳 90000 这个数值。

短数据类型与长数据类型相比，其优点是可以节约程序的存储空间，提高程序运行效率。但它能表示的数据范围较小，容易出现数值溢出错误。对于单个的数据（不用于数组），如果使用频率不是特别高，那么短数据类型能够提高的效率是非常有限的，甚至可以忽略。这时，可以尽量考虑使用长数据类型，以避免可能发生的错误。
...
当 For 循环迭代次数为 0 (N=0) 时：
- 普通隧道： 输出端会输出该数据类型的默认值。例如，文件引用句柄会输出“Not A Refnum”，导致后续的“关闭文件”函数失效，从而引发内存泄漏；错误簇会输出“无错误”，导致之前的错误被吞掉。
- 移位寄存器： 输出端会直接原封不动地输出输入端的值。这保证了即使循环不执行，引用句柄和错误信息也能正确传递到下游代码。
...
这个错误的原因在于：簇中元素在数据线中的排序，并不一定和界面上看到的顺序是一致的。界面上元素控件的位置可以被任意挪动，数据的顺序也可以被调整。在簇的右键菜单中选择 "重新排序簇中控件"，即可设置元素数据的顺序：
...
* 使用严格类型定义（Strict Type Def）：凡是用到簇的地方，都应当为其创建一个“严格类型定义”控件（.ctl 文件）。这不仅能保证所有簇中元素的类型、顺序完全一致，避免了上图中的错误。而且，这还能保证它们的外观属性一致。一旦簇结构发生变化，只需修改 .ctl 文件，所有引用的 VI 都会自动更新，一劳永逸。
* 按名称捆绑/松绑： 尽可能使用“按名称捆绑”和“按名称松绑”函数。这能让代码具有自解释性，即便簇内顺序改变，只要名字没变，代码就不会出错。
...
另一种解决方案是使用[功能全局变量](pattern_global_data#功能全局变量)（FGV）（也称为 Action Engine）。FGV 是一个设置为“不可重入执行”的 SubVI，它利用 While 循环中未初始化的移位寄存器来存储数据。它的好处是：
- 天然线程安全： 由于 VI 是不可重入的，LabVIEW 引擎会自动串行化对该 VI 的所有调用，相当于天然加了锁，无需手动处理信号量。
- 封装性好： 数据的读写逻辑都封装在 VI 内部，避免了全局变量满天飞导致的维护噩梦。
