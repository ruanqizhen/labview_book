...
有时候，编程语言的设计者也会发现一些无法容忍的历史遗留设计失误，即使破坏兼容性也要做修改。一个著名的例子是 Python 从 2.0 升级到 3.0。表面上看，这次升级似乎只是修正了一些设计缺陷（如将字符串默认类型彻底改为 Unicode，将 print 语句改为函数等）。但在底层，这绝对不是“小变动”。特别是 Unicode 的默认化，直接触动了所有底层文件 I/O、网络通信和第三方库的神经，导致几乎所有 Python 2 的遗留代码在 Python 3 下都无法直接运行。正因为付出了打破向后兼容性的巨大代价，整个 Python 社区阵痛了十来年的时间，才促使庞大的第三方生态彻底完成迁移。

相比之下，LabVIEW 作为一个工业级软件，背负着极其沉重的历史包袱。LabVIEW 年年出新版本，因为每个新版本只是修修 bug 或扩展一些功能，所以依然可以保证向前兼容，不会有太大升级阻力。但即便如此，因为 LabVIEW 不能保证向后兼容，不能用低版本的 LabVIEW 打开高版本 LabVIEW 保存的 VI，LabVIEW 的兼容性还是受到了不少批评的。LabVIEW 有一些广为诟病的问题，由于修改这些问题后无法保证兼容性，所以 NI 公司迟迟无法下决心做更正。比如支持 Unicode，尽管支持 Unicode 本身并不困难，可是一旦支持了 Unicode，原本那些依赖非 Unicode 编码的程序就都无法正常运行或显示，所以 LabVIEW 也不敢轻易就变换编码。
...
- 实现多参数函数（这个方法有个专用名叫函数柯里化 Currying）。因为 Lambda Calculus 规定一个函数只能接受一个参数，如果想实现类似 `f(x, y) = x + y` 的多参数函数，必须将其转换为“返回函数的函数”。比如实现函数 `f(x, y, z) = ((x, y), z)` 可以使用如下的代码： `λx.λy.λz.xyz`
- 逻辑运算中的“真”被定义为：`TRUE ≡ λx.λy.x`。这里对于“真”的定义是：输入两个参数，选择并返回第一个。我们来推算一下（注意 Lambda 演算的调用是左结合的）：
...
假设我们有一组正方形，它们的边长分别是 2, 3, 4, 5。我们可以把这些数据放在一个列表里，Scheme 中可以使用 list 函数生成一个列表：`(list 2 3 4 5)`。接下来我们可以利用 map 函数计算每一个正方形的面积。map 函数接收两个参数：第二个参数是一个列表（List），表示需要被处理的一组数据； 第一个参数是一个函数，表示如何处理列表中的每一个元素。这样，我们可以利用刚刚定义的 square 函数来计算正方形面积： `(map square (list 2 3 4 5))` 运算结果是： `(list 4 9 16 25)`。这里就是把函数 square 作为一个参数传递给了函数 map 。
...
现在很多主流编程语言也都支持 lambda 函数了，用法与 Scheme 中的函数类似。如果在其它语言中已经学习过了，上面的示例会更容易理解。
...
* 数据传递机制的不同： LabVIEW 使用显式的数据线在各个节点之间传递数据，数据流直接决定了程序的执行顺序。而在 Scratch 中，没有独立的数据线。它通过积木的物理嵌套（将椭圆形的数据积木嵌入到执行积木的参数凹槽中）来实现局部的参数传递与求值；对于跨越多个独立代码块的数据共享，Scratch 则需要依赖创建显式的“变量”（类似于局部或全局变量）来实现。
