...
假如用户在观察了波形全图后，决定选取波形中第 3 秒和第 5 秒之间的数据进行观察。放大后需要显示的数据长度为 2 秒，对应于原始数据的第 30,000～50,000 个点。这 20000 个点也不需要全部在波形图上显示，可以对它们进行重采样，采样频率为 100。这样，重采样的数据还是有 200 个点，表示原数据第 3～5 秒的内容。
