...
除了界面部分，这个组件还包括自定义的一些行为。最初，这个控件只是显示一组路径给用户。这个组件只使用列表框列出一组路径，用于编辑路径的文本框和浏览路径按钮是隐藏不见的。当用户用鼠标点击了列表框上的某一条路径，表示用户需要编辑这一路径。此时，组件需要把文本框和浏览路径按钮在鼠标点击的这一行显示出来，并使用这一行所显示的路径作为文本框的默认值，供用户编辑调整。还有其它一些行为：如，点击浏览按钮要弹出路径浏览对话框；点击向下移动按钮要使选中的路径向下移动一位等。
...
上述“同一功能对应多种界面”的需求，本质上是软件工程中经典的 MVC (Model-View-Controller) 或 MVP 架构模式。

在大型 LabVIEW 项目中，最成熟的做法是让“界面 VI (View)”和“功能 VI (Model/Controller)”作为两个独立的并行循环或独立 VI 运行，而不是相互嵌套调用：
1. 界面 VI (View)： 仅负责捕捉用户的点击，并通过队列 (Queue) 或用户事件 (User Event) 发送抽象的命令（如“请求生成随机数”）给功能 VI，本身不处理任何逻辑。
2. 功能 VI (Logic)： 在后台循环接收命令，执行复杂的算法。计算完成后，再通过预先获取的控件引用，或向界面 VI 发送“更新界面”的事件，将结果（如随机数）显示出来。

这种彻底解耦的设计，使得我们可以随时丢弃或替换几十个不同的界面 VI，而功能 VI 连一根线都不需要修改。
...
如果不使用 `打开 VI 对象引用` 函数也可以使用下面的标准做法获取对象的引用：
1. 通过界面 VI 的引用，获取其 前面板 (Front Panel) 引用。
2. 读取前面板的 控件[] (Controls[]) 数组属性。
3. 使用一个 For 循环遍历这个数组，读取每个控件的 标签.文本 (Label.Text) 属性。
4. 与需要的字符串（如 "Button1"）进行比对。如果匹配，则将该通用控件引用“转换为更具体的类（如布尔型）”并保存下来。
