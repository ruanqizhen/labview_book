...
程序运行的思路大致如下：程序从左至右，读出数据和运算符，把数据和运算符先保存到栈中，然后继续读直到下一个运算符，再判断是否可以计算之前记录的数据和运算符，如果可以就计算，保存结果，然后继续。
...
需要注意，使用字符串数据作为条件结构的输入条件时，`"0".."9"` 这个条件设定是无法匹配 "9" 这个字符的（字符串范围的上限不含端点），必须额外标明，这个条件分支需要处理字符 "9"。这是使用字符串类型数据作为选择条件的一个不便之处，详细说明可以参考[条件结构](structure_cond_seq#其它数据类型的条件选择)一节中的说明。
