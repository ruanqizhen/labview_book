...
本书之前介绍过的各种控件，相对来说，处理的数据都是比较单一的，比如按钮控件或灯泡控件就是用来表示布尔类型的数据。通常，在程序里也就只会用到这些控件的值，需要调用属性节点设置外观等属性的情况是比较少呢。但是，我们将要介绍的这几个控件相对就复杂一些，所需的数据比较多。比如，当我们使用列表控件的时候，都至少需要提供两种数据，一是列表中每个条目的文字，是一个字符串数组数据；二是选中的条目，如果单选应该是个整数，多选就会是一个整数数组。如果控件选择这两种数据之一作为控件的数据，那么另一个数据就只能通过控件的属性来设置了。所以，在程序中使用到列表等控件的时候，通常都不会只使用到它的接线端和局部变量，还会要使用到它的属性节点。

下面通过一些实例来解释如何使用这几个控件。

### 列表控件的基础设置

列表控件与 [下拉列表控件](data_custome_control) 比较相似，区别在于下拉列表控件只显示列表中被选中的条目，而列表控件可以显示多个条目，更方便用户进行挑选。另一点不同是列表控件还可以支持多选，用户可以在其右键菜单中切换“选择模式”。列表控件上被选中的条目会被高亮显示。

列表控件（以及树形控件等）的每个条目的前端，还可以为其选择并显示出一个图标。选中控件的右键菜单“显示项 -> 符号”，就可以把选取的图标显示出来：
...
上图中最后一个符号图标其实不是一个“图标”，而是分割线。图中列表控件中，第一个条目和第二个条目之间就是一个分割线。
...
程序使用了列表控件列出所有的文件，实现如下：

![](images_2/z237.png "选取文件夹中的文件")

程序分成两个部分：循环结构用于把用户选中的文件名显示在“selected item”控件上；循环结构左边的程序用于读取一个文件夹中的所有文件并显示在列表控件上。程序中使用到的“List Folder”函数可以读取一个文件夹下的所有子文件夹和文件，把它们的名字返回出来。通过设置列表控件的“ItemNames”，让列表控件把这些条目显示出来。程序同时还为每个条目设置了一个符号图标，如果是文件夹，就设置一个文件夹图标，如果是 VI 文件，就设置一个 VI 图标。程序在设置符号图标时使用了一个“Listbox Symbol Ring”常量来选取图标，这个常量在函数选板“编程 -> 对话框和用户界面”上。

程序的运行结果如下：

![](images_2/z238.gif "选取文件夹中的文件")

在应用程序中，常常还需要动态的设置列表控件中的条目，比如可以允许用户点击按钮或用鼠标把某些条目从一个列表控件拖到另一个列表控件中去。本书会在讲解了[事件结构和程序界面](pattern_ui) 在后再介绍这种复杂的用户界面交互的实现方法。
