...
It sets the values of several constants based on the current VI’s path. This VI includes 'error in' and 'error out' controls, which are used solely for [managing the program's flow](structure_cond_seq#the-intangible-over-the-tangible) rather than transmitting actual data. In practical applications, this initialization VI is first invoked to set necessary data for all constants. After this setup, the constant global variables can only be read within the program:
